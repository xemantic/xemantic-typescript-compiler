 ...
 * tsgo's by design at the `"6.0"` default `simulatedVersion` (TS5107/TS5101 — the
...
class ProjectTsconfigOptionAnchorTest {
    ...
    fun `alwaysStrict false anchors at its value token in the project's tsconfig`() {
        val row = build(root to oneLine(""""alwaysStrict": false""")).singleOrNull { it.code == 5107 }
        assert(row != null)
    ...
    fun `esModuleInterop false anchors at its value token`() {
        val row = build(root to oneLine(""""esModuleInterop": false""")).singleOrNull { it.code == 5107 }
        assert(row != null)
        assert(row.fileName == root)
        assert(row.line == 1)
    ...
    fun `allowSyntheticDefaultImports false anchors at its value token`() {
        val row = build(root to oneLine(""""allowSyntheticDefaultImports": false""")).singleOrNull { it.code == 5107 }
        assert(row != null)
        assert(row.fileName == root)
        assert(row.line == 1)
    ...
    fun `downlevelIteration anchors at its key token including the quotes`() {
        val row = build(root to oneLine(""""downlevelIteration": true""")).singleOrNull { it.code == 5101 }
        assert(row != null)
        assert(row.fileName == root)
        assert(row.line == 1)
    ...
    fun `baseUrl anchors at its key token`() {
        val row = build(root to oneLine(""""baseUrl": "."""")).singleOrNull { it.code == 5101 }
        assert(row != null)
        assert(row.fileName == root)
        assert(row.line == 1)
        assert(row.character == 96)
    ...
    fun `module amd anchors at its quoted value`() {
        ...
        val row = build(root to config).singleOrNull { it.code == 5107 && it.message.contains("module=AMD") }
        assert(row != null)
        assert(row.fileName == root)
        assert(row.line == 1)
    ...
    fun `target es5 anchors at its quoted value`() {
        ...
        val row = build(root to config).singleOrNull { it.code == 5107 && it.message.contains("target=ES5") }
        assert(row != null)
        assert(row.fileName == root)
        assert(row.line == 1)
    ...
    fun `moduleResolution node anchors at its quoted value`() {
        ...
        val row = build(root to config).singleOrNull { it.code == 5107 && it.message.contains("moduleResolution=node10") }
        assert(row != null)
        assert(row.fileName == root)
        assert(row.line == 1)
    ...
    fun `a multi-line compilerOptions anchors each option on its own line`() {
        ...
        val strict = diagnostics.singleOrNull { it.code == 5107 && it.message.contains("alwaysStrict=false") }
        val interop = diagnostics.singleOrNull { it.code == 5107 && it.message.contains("esModuleInterop=false") }
        assert(strict != null)
    ...
    fun `a CRLF tsconfig counts lines by CRLF and columns without the CR`() {
        ...
        val row = build(root to config).singleOrNull { it.code == 5107 }
        assert(row != null)
        assert(row.fileName == root)
        assert(row.line == 4)
    ...
    fun `an option inherited through extends anchors at the root's compilerOptions key`() {
        ...
        val inherited = diagnostics.singleOrNull { it.code == 5107 && it.message.contains("alwaysStrict=false") }
        val own = diagnostics.singleOrNull { it.code == 5107 && it.message.contains("esModuleInterop=false") }
    ...
    fun `inherited key-anchored options fall back to the root's compilerOptions key too`() {
        ...
        val rows = diagnostics.filter { it.code == 5101 || it.code == 5107 }
        assert(rows.size == 3)
    ...
    fun `a root override anchors at the root's own value and silences the base's setting`() {
        ...
        val rows = diagnostics.filter { it.code == 5107 }
        assert(rows.size == 1)
