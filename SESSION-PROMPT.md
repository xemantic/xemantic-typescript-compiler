...
**Concrete starting points for the next architectural session (queue item B60.x):**
- **Blocker #1 step 1 (control flow narrowing)**: Build flow-graph infra in binder (Flow.kt already has FlowGraph; extend to per-block reaching defs + narrow types per AST position). Wire into TS2454/TS2339/TS2774/TS2839 emission sites. ~4-5 sessions total, +60-100 tests. Highest absolute yield.
- **Blocker #2 nested-T inference (B52.2)**: Extend `tryInferSingleTypeParamFromArgs` (Checker.kt:43925) gate (e) to handle `Type.Object` params with one `TypeParam(T)`-typed property. Mirrors 17.38's all-TypeParam case for single-property containers. ~1 session, ~3 tests. Smallest scope.
- **Blocker #3 per-file scope**: Add per-file scope construction behind a flag (start at Checker.init's `mergeSymbolTable` — split "true globals" from cross-file module locals). ~3+ sessions, ~30 tests. Highest risk.
