...
- **10,077 tests**, 7,453 passing (73.9%), 2,624 failing
- **Session 2026-03-23e**: +33 tests: TS2322 expanded — TypeReference support for named types, ReturnStatement checking against function return types, null/undefined not assignable to named types under strict null checks (+30), Kotlin property initialization order fix for `strictNullChecks` val, TS2322 suppression when TS2304/TS2314 exists for same type name, `matchReturnTypeInAllBranches` return keyword type checking (+1), plus method/getter return type checking, labeled statement traversal
