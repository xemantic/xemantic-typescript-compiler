...
- **Node16/Node18/Node20/NodeNext are all CJS for `.ts` files**: `isESModuleFormat` must return `false` for plain `.ts` files — only `.mts`/`.mjs` are ESM. ALL files in these modes get CJS module treatment (`__esModule`). Use `ModuleKind.isNodeNext` helper for checks.
