...
- **OBJECT_PROTOTYPE_PROPERTIES filter in `propertiesRelatedTo`**: When the lib `Object` interface resolves to a real Type.Interface (i.e. after Variable+Interface merge), its properties (`constructor`, `toString`, `valueOf`, …) appear in `target.properties`. Without filtering them in `propertiesRelatedTo`, any `{a:string}` source fails to satisfy `Object` because `constructor` is "missing". The filter must mirror the one in `collectMissingProperties` and `getMissingRequiredPropertySymbol` and the TS2344 missing-property loop in `checkConstraintsForTypeArgs` — every "is this property missing" check needs to skip prototype members.
...
- **Array-vs-array outer TS2322 suppression for array literals**: Var-decl and assignment paths must skip the outer "Type 'X[]' is not assignable to type 'Y[]'." when `init`/`expr.right is ArrayLiteralExpression` and target is a `Type.Reference` to Array. `checkArrayLiteralElementExcessProps` and related walkers already emit per-element TS2322/TS2353 at the specific offending element; the outer diagnostic would duplicate them. Matches TypeScript's baseline for `contextualTyping21_ts` (`[{id:1}, 1]` → `{id:number}[]`). Without the suppression, removing the stale `canUseTypeEngine` array-vs-array bail-out fires two TS2322s for the same assignment.
- **`getTypeFromTypeNode` cache bypass for context-sensitive resolution**: `nodeTypes` cache must be bypassed when ANY context-stack is active that affects type-name resolution: `currentTypeParamScope` (class/method type-param scope) AND `inferenceNamespaceStack` (lazy-resolution namespace scope). A type node may resolve to different `Type` values under different contexts, and a stale cache entry (e.g., `errorType` from an earlier eager pass before the context was active) would shadow the correct context-aware resolution. Pattern: any new "lazy resolution context stack" added in the future must be checked here too — failure mode is silent (cache hit returns wrong type, downstream type checks pass falsely).
