...
**Phase 4 — Checker buildout.** 8,990 / 10,078 tests passing (~89.21%). _Round 52 iter10 (2026-05-27, **B83.4d-quick callback-return-tp inference branch landed, net-zero infra**). New branch in `tryInferSingleTypeParamFromArgs` pass-2 (Checker.kt:~54222): when callback shape is `(x: otherTp) => <mentions currentTp>` where otherTp ∈ tps is already in mapperPairs, push `currentLocalTypes[lp.name] = mapperPairs[otherTp]` and call `getTypeOfExpression(lambdaBody)` to gather currentTp candidate. Narrow gate: concise-body lambdas only (NOT Block), single Identifier param, no annotation, anonymous Type.Object callback shape. Net-zero — confirms the infra is in place but real failing-test flips require additional pieces (likely the recon-noted FunctionExpression contextual-param Blocker #2). Round 52 net so far: +1 / 10 iter._
