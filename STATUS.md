...
**Phase 4 — Checker buildout.** 8,867 / 10,078 tests passing (~87.98%). _Round 15 (2026-05-20): 5 feature commits, +5 net via B61.1 (TS2300 lib-shadowing) + B61.2 (TS1351 numeric-id) + B61.3 (TS1005 deferred var-decl emit) + B61.4 (TS1029 ImportEqualsDeclaration modifier order, net-zero) + B61.5b (TS2694 narrow non-exported import-equals member). Session reached 20+ iteration cycles, including reverted B52.2/B61.5 attempts and a stale-skip-log audit._

**Round 15 (2026-05-20, +5 net — B61.x parser/checker stack).** /goal session after round 14. Pool empty at start (`find_candidates.py --fresh` returned 0/0/0). Per anti-loop rule, surveyed broader candidate space:
...
- **B61.5b (+1)**: Narrow TS2694 for non-exported namespace member in import-equals. AST-level export check (Function/Class/Interface/TypeAlias/Enum/Module modifier scan + VariableStatement body scan). Sub-namespace Module-flag gate prevents FP on dotted-namespace form. Flips `importDeclWithDeclareModifier_ts`.

Round 15 totals: 5 feature commits + status/plan doc commits, +5 net tests (8862 → 8867). Surgical wins from adjacent diagnostic-pattern work (lib-shadowing TS2300 + scanner TS1351 + parser TS1005 + ImportEqualsDeclaration TS1029/TS2694).
