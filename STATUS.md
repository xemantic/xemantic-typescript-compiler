...
**Phase 4 — Checker buildout.** ~8,340 / 10,078 tests passing (~83%).
...
2. **Blocker #1**: Full control flow narrowing (~2–4 sessions, +60–100 tests).
   ...
   - **Step 2c (cont, next)**: Remaining wire-ups: TS2454 via flow-graph definite-assignment (replace ad-hoc walker — note 17.1c session warned a snapshot/restore approach regresses -7), TS2774 PropertyAccessExpression operand support (needs body-walk path matching + parameter-type lookup for callable params), `&&`-chain narrowing wired into TS2774 emission (`uncalledFunctionChecksInConditional2_ts`), FlowAssignment-RHS narrowing (medium risk — could over-narrow legitimate union-source TS2322 cases), `x.constructor === Class` narrowing (`typeGuardConstructorDerivedClass_ts`).
