...
**Phase 4 — Checker buildout.** 8,946 / 10,078 tests passing (~88.77%). _Round 43 (2026-05-25, in-progress /goal session) — 11 commits landed, all net-zero on suite. Per user direction (AskUserQuestion), committed to architectural-blocker focus. Round 43 iters: iter1+2 broadening (SubsequentVarTypes accessor bodies, JSDocParamTags accessor bodies); iter3 narrowByAssertCall for assert-function narrowing; iter4 mirror to FollowLoopEntry; iter5 narrowBySwitchClause for switch-case literal narrowing; iter6 mirror to FollowLoopEntry; iter7 CLAUDE.md gotcha for flow-narrowing infra; iter8 PropertyAccess callee in narrowByAssertCall; iter9 PropertyAccess callee in narrowByCallPredicate; iter10 switch-clause discriminant-parent narrowing (switch (x.kind) narrows x); iter11 optional-chain receiver narrowing (if (obj?.x) narrows obj). All 9 architectural commits net-zero — building Blocker #1 (control-flow narrowing) infrastructure. New emission-site wiring needed for test flips._
