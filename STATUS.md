...
**Phase 4 — Checker buildout.** 8,866 / 10,078 tests passing (~87.97%). _Round 15 (2026-05-20): 3 feature commits, +4 net via B61.1 (TS2300 lib-shadowing) + B61.2 (TS1351 numeric-id) + B61.3 (TS1005 deferred var-decl emit). Session reached 12+ iteration cycles, including a reverted B52.2 attempt and a stale-skip-log audit._

**Round 15 (2026-05-20, +4 net — B61.x parser/checker stack).** /goal session after round 14. Pool empty at start (`find_candidates.py --fresh` returned 0/0/0). Per anti-loop rule, surveyed broader candidate space:
...
- **B61.2 (net-zero infra)**: TS1351 emission for identifier immediately following numeric literal ending in `.` (e.g. `1.toString`). Detection in `parseNumericLiteral` checks (a) text ends with ".", (b) next token is Identifier, (c) no whitespace between. Pairs with B61.3.
- **B61.3 (+2)**: TS1005 ',' expected at the deferred position (next token AFTER trailing identifier) in var-decl-list recovery context. When the previous decl's init is a `NumericLiteralNode` ending with '.', parse the next declarator FIRST then emit TS1005 — landing on the `(` instead of the identifier. Flips `numericLiteralsWithTrailingDecimalPoints01/02_ts` (when paired with B61.2's TS1351).

Round 15 totals: 3 feature commits + status/plan doc commit, +4 net tests (8862 → 8866). Demonstrates that surgical wins from adjacent diagnostic-pattern work (parser scanner + var-decl recovery) can compound when the right pieces land together.
