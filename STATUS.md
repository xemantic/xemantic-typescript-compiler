...
**Phase 4 — Checker buildout.** 8,946 / 10,078 tests passing (~88.77%). _Round 42 (2026-05-25, in-progress /goal session) — 9 net-zero commits + 1 chore + 2 reverts. iter1 B52.2 (mixed-Type.Object inference gate relaxation); iter2 queue chore (closed stale B60.11 checkbox); iter3 expressionTrueEnd NoSubstitutionTemplate; iter4 computed-prop nested member recursion; iter5 findClassesInStatement broader stmt recursion; iter6 checkStmtForPrivateFieldAccess broader stmt coverage; iter7 TaggedTemplate in private-field walker; iter8 checkAmbientInExpr ClassExpression accessors + TaggedTemplate; iter9 walkForOfNonIterableInExpr broader expression coverage. Reverts: iter4+5-combo (-1 from isLiteralLikeExpr wrapper unwrap suppressing TS1166/TS1169); iter10 (-16 from checkTypeAsValueInStatement broader stmt over-emitting TS2693/TS2708). 11 consecutive rounds (32-42) at ~0 net flips per CLAUDE.md anti-loop rule; next session MUST commit entirely to one architectural blocker._
