...
**Phase 4 — Checker buildout.** 8,930 / 10,078 tests passing (~88.61%). _Round 26 (2026-05-22): /goal session (10-iteration target) — 2 feature commits + 1 reverted, +2 net via B69.1+B69.2._

_Round 26 (2026-05-22, +2 net via 2 feature commits + 1 reverted)_:
...
- **B69.3 (reverted)**: Narrowed `checkPropertyAccessAssignment`'s null-skip gate to fire only when `!strictNullChecks`. Flipped 2 errors on `chainedAssignment3_ts` (chained `a.id = b.value = null`) but regressed 1 other test, net -1. Test still needs 2 more diagnostics for outer chained class A→B mismatch. Reverted.
