...
**Phase 4 — Checker buildout.** 8,948 / 10,078 tests passing (~88.79%). _Round 47 iter2 (2026-05-25, **+1 via B52.3**). `genericCallWithObjectLiteralArguments1_ts` flipped via three combined changes: (a) extending `instantiateType` to walk anonymous Type.Object property-bag members (substitutes TypeParam-typed property types for explicit `<T>` type args), unblocking per-property TS2322 + TS6500 elaboration that was already in place; (b) "return after per-property emission" to suppress now-redundant TS2345 at bare-T anchor; (c) candidate-priority sort so bare-T/Array-T candidates outrank object-literal-of-T (union) candidates — the bare anchor wins when both shapes contribute to the same TypeParam._
