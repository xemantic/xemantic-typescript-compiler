#!/usr/bin/env bash
# (P18.201) the 8-profile BEFORE/AFTER binary grid for (CHK.171) R1 — the constructor/setter frame and nested function
# frames keep the class / outer type-parameter scope. The census predicted +0 on project/harness/rxjs for this arm.
#
# BEFORE is the orchestrator's snapshot of the (P18.200) landed binary (Checker.class md5 e534ca5f,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18201/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18201/classes-before
