#!/usr/bin/env bash
# (LEGACY.1) step (d1) — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The change anchors deprecation/removed-option rows at the
# tsconfig VALUE on the project path; no profile sets a removed option, so no such row exists to
# move and both halves are controls by construction. The corpus screens and the suite (core +
# project modules) are the gates.
#
# REFUSES when the two arms' TypeScriptCompiler.class AND Checker.class are byte-identical (a
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-104-orch; mkdir -p "$OUT/grid"
...
# (P18.104) touches TypeScriptCompiler (and maybe the Checker); refuse only when NEITHER moved.
same=1
for cls in TypeScriptCompiler Checker; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' TypeScriptCompiler and Checker classes are both byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
