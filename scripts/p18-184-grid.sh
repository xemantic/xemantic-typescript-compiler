#!/usr/bin/env bash
# (P18.184) the 8-profile BEFORE/AFTER binary grid for (CHK.153) — the callee receiver of a callback's
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18184/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18184/classes-before
