#!/usr/bin/env bash
# (LEGACY.1) step (j3) — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The change deletes the tslib ES5 helper arms, which tsgo never
# reaches at a written es5 (it checks as es2015+); every profile is es2020+, so neither half can move —
# a difference here is a FINDING. The corpus screens and the pins are the gates.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-112-orch; mkdir -p "$OUT/grid"
...
# (P18.112) touches the Checker (maybe Transformer); refuse only when NONE moved.
same=1
