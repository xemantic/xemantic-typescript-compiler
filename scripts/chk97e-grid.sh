#!/usr/bin/env bash
# (CHK.97) stage 3 DELIVERABLE D2 — the 8-profile BEFORE/AFTER binary grid.
#
# BEFORE is HEAD cab774f28 (D4 landed), AFTER is the D2 tree.  EXPECT A CONTROL,
# NOT A GATE: tsc's own 78 sources carry essentially no union callee with TWO
# overloaded members, so `added=0 removed=0` here proves INERTNESS and nothing
# about coverage ((CHK.124)'s lesson).  The counting arm beside it is what says
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk97e/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk97e/classes-before
AFTER=build/bench/chk97e/classes-after
