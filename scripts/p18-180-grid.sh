#!/usr/bin/env bash
# (P18.180) the 8-profile BEFORE/AFTER binary grid for (CHK.150) rung (1), X2 — structural
# contextual inference between DIFFERENT object types (a `Subscriber<T>` return inferred from an
# `Observer<T>` context, and an object literal against a generic-interface instantiation).
#
...
#
# BEFORE is the orchestrator's snapshot of HEAD 4c090d29 (Checker.class md5 124ca26e, rebuilt and
# re-verified before the round started); AFTER is the built tree.
# REFUSES when the arms' Checker.class are byte-identical, when a capture is empty or TRUNCATED,
# and below 8 profiles.
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18180/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18180/classes-before
