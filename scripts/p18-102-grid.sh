#!/usr/bin/env bash
# (LEGACY.1) steps (a)+(b) — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT TWO CONTROLS, AND COUNT THEM. The change deletes DEAD System-module helpers from the
# Transformer; every profile is `module: commonjs`/esnext, so neither the `--listAll` half nor
# the EMIT half can move — `added=0 removed=0` and 78/78 identical are statements about the
# profiles. The corpus emit screen and the suite are the gates.
#
# REFUSES when the two arms' Transformer.class is byte-identical (a self-comparison prints a
# clean bill of health — CLAUDE.md rounds 853/946), when a capture is empty or TRUNCATED, and
# below 8 profiles.
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-102-orch; mkdir -p "$OUT/grid"
...
# (P18.102) touches the Transformer only.
same=1
for cls in Transformer; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' Transformer.class is byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
