#!/usr/bin/env bash
# (CHK.133)(a)+(c) — the 8-profile BEFORE/AFTER binary grid for `Signature.thisType` and the
# call-site TS2684 check (tsc's `getSignatureApplicabilityError` `this` leg).
#
...
#
# BEFORE is HEAD f847f6ed6, AFTER is the (CHK.133) tree.  Two snapshotted class
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk133/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk133/classes-before
AFTER=build/bench/chk133/classes-after
