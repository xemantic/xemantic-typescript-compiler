#!/usr/bin/env bash
# Round 936 — the 8-profile `--listAll` output grid for LATE-BOUND computed keys.
#
...
# (round 805).
# Usage: scripts/round936-grid.sh <before-Checker.kt> <after-Checker.kt>
set -uo pipefail
cd /home/claude/git/xemantic-typescript-compiler

BEFORE_SRC="${1:?usage: round936-grid.sh <before-Checker.kt> <after-Checker.kt>}"
AFTER_SRC="${2:?usage: round936-grid.sh <before-Checker.kt> <after-Checker.kt>}"
CK=xemantic-typescript-compiler-core/src/commonMain/kotlin/Checker.kt
OUT=build/bench/round936-grid
