#!/usr/bin/env bash
# (P18.181) the 8-profile BEFORE/AFTER binary grid for (CHK.150) rung (2), X3 — a UNION-typed
# parameter as the contextual type of a call's return (callee type parameter inferred through a
# union constituent).
#
# BEFORE is the orchestrator's snapshot of the (P18.180) landed binary (Checker.class md5 8dfaf957,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18181/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18181/classes-before
