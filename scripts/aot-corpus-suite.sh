...
# untouched. EXPECT 13 FAILURES IN BOTH ARMS (10 before round 840 added two
# launcher pins, then 13 once (AOT.4)(b) pinned the native image's entry point
# too — it reads build.gradle.kts, so it needs the same exploded dir), and only
# these: 11 in
