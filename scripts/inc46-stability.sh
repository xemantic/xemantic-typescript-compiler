#!/usr/bin/env bash
# (INC.46) STEP 2 — the STABILITY RATE of the exported-signature fingerprint against a
# REAL EDIT CORPUS: what fraction of actual commits to tsc's own `src/compiler` leave
# every touched file's exported signature unmoved.
#
...
#
# Usage: scripts/inc46-stability.sh [<cases> [<projectDir>]]
set -euo pipefail
cd "$(dirname "$0")/.."
ROOT="$PWD"
CASES="${1:-40}"
...
exec java -Xmx6g -cp "$CP" \
  com.xemantic.typescript.compiler.project.Inc46StabilityMainKt "$CORPUS" "$SCRATCH"
