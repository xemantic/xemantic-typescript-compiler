#!/usr/bin/env bash
# (BUG.3) round 923 — the 8-profile `--listAll` output grid.
#
# TWO BINARIES, not two flags: the change has no runtime switch, so the arms are
# HEAD's `Checker.kt` and the round's. The claim under test is that (BUG.3) is
# invisible to a compile — `typeCaptureThisClass` is reachable only from
# `typeCaptureVisit`, which returns at its first line when no capture was
...
cp "$CHK" "$OUT/Checker.round923.kt"
git show HEAD:"$CHK" > "$CHK"
./gradlew compileKotlinJvm > "$OUT/build.A.log" 2>&1 || { echo "BUILD A FAILED"; cp "$OUT/Checker.round923.kt" "$CHK"; exit 1; }
grep -aq 'BUILD SUCCESSFUL' "$OUT/build.A.log" || { echo "BUILD A NOT SUCCESSFUL"; cp "$OUT/Checker.round923.kt" "$CHK"; exit 1; }
capture head

echo "=== arm B: round 923 ==="
cp "$OUT/Checker.round923.kt" "$CHK"
...
for d in "${profiles[@]}"; do
  ...
  added=$(comm -13 "$OUT/$name.head.diag" "$OUT/$name.r923.diag" | wc -l)
  removed=$(comm -23 "$OUT/$name.head.diag" "$OUT/$name.r923.diag" | wc -l)
  n=$(wc -l < "$OUT/$name.r923.diag")
