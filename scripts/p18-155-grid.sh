#!/usr/bin/env bash
# (P18.155) — the 8-profile BEFORE/AFTER grid for TS7009 at a NON-IDENTIFIER `new` callee.
#
# THE ROW-SET DIFF IS THE GATE HERE, not a control: this round ADDS a diagnostic, so
# `added` can move and every added row on tsc's own sources is a FALSE POSITIVE by
# definition — those sources compile clean under tsc. `new X.Y()` is ordinary in them,
# which is exactly the population the rule reasons about. `fullDiffLines` is kept as the
# finer instrument and the emit `diff -r` as the control that the transform path is inert.
#
# REFUSES when the two arms' Checker.class is byte-identical (rounds 853/946).
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-155-orch; mkdir -p "$OUT/grid"
