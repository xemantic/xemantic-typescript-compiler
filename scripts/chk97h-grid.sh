#!/usr/bin/env bash
# (CHK.97) D3 — the 8-profile BEFORE/AFTER binary grid for the DIFFERING-signature
# union contextual signature (TS7006 emission + arity-filtered members).
#
...
#
# BEFORE is HEAD f7241883e, AFTER is the (CHK.97) D3-differing tree.  Two snapshotted class
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk97h/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk97h/classes-before
AFTER=build/bench/chk97h/classes-after
