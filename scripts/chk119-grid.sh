#!/usr/bin/env bash
# (CHK.119) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk119/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk119/classes-before
AFTER=build/bench/chk119/classes-after
