#!/usr/bin/env bash
# (WARM.12) round 865 — the 8 dashboard profiles captured from TWO class
...
# `FlowCensus` (a class that did not exist before this round) and the BEFORE dir
...
#
#   usage: scripts/round865-grid.sh <before-class-dir> <after-class-dir> [outdir]
set -uo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
BEFORE="${1:?before class dir}"
AFTER="${2:?after class dir}"
OUT="${3:-build/bench/r865-grid}"
mkdir -p "$OUT"
...
javap -p -cp "$AFTER" com.xemantic.typescript.compiler.FlowCensus >/dev/null 2>&1 || {
  echo "REFUSED: the AFTER dir has no FlowCensus — it predates (WARM.12)" >&2; exit 1; }
if javap -p -cp "$BEFORE" com.xemantic.typescript.compiler.FlowCensus >/dev/null 2>&1; then
  echo "REFUSED: the BEFORE dir HAS FlowCensus — the two arms are the same build" >&2; exit 1
fi
