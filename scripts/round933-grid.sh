#!/usr/bin/env bash
# Round 933 — the 8-profile `--listAll` output grid for the template computed-key fix.
#
# TWO BINARIES, not two flags. This change has no in-binary arm (it is one `when`
# arm plus one delegation), so the grid rebuilds the PRE-933 source, captures, and
# rebuilds the fixed one — the shape round 813 asks for: a whole-output diff of the
...
# (round 805).
# Usage: scripts/round933-grid.sh <before-Checker.kt> <after-Checker.kt>
set -uo pipefail
cd /home/claude/git/xemantic-typescript-compiler

BEFORE_SRC="${1:?usage: round933-grid.sh <before-Checker.kt> <after-Checker.kt>}"
AFTER_SRC="${2:?usage: round933-grid.sh <before-Checker.kt> <after-Checker.kt>}"
CK=xemantic-typescript-compiler-core/src/commonMain/kotlin/Checker.kt
OUT=build/bench/round933-grid
