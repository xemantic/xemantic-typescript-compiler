#!/usr/bin/env bash
# (CHK.134) decomposition (2) — the 8-profile BEFORE/AFTER binary grid for `.bind` on a
...
#
# EXPECT A CONTROL.  The reach census over the instrumented binary read every `.bind`
# miss on every profile REFUSED — 3/18/9/5/3/3/3/3 `non-strict` (tsc's own tsconfigs set
# `"strictBindCallApply": false`) plus 2-6 optional-chain UNION receivers — and ZERO sites
# on marked, cronstrue and the 2,400-file project, so nothing here can move a row; the
# gate for the synthesis is `FunctionBindTest` (53 pins) and the ablation arms.
#
# BEFORE is HEAD 408b6162, AFTER is the (CHK.134)(2) tree.  Two snapshotted class dirs,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk134b/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk134b/classes-before
AFTER=build/bench/chk134b/classes-after
