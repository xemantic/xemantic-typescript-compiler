#!/usr/bin/env bash
# (P18.186) the 8-profile BEFORE/AFTER binary grid for (CHK.154)(b) — `new Derived(...)` is checked
# against the derived class's OWN construct signatures, not the base constructor's as well. A
# false-NEGATIVE fix: it may ADD rows, and every added row must be a tsgo row.
#
# BEFORE is the orchestrator's snapshot of the (P18.185) landed binary (Checker.class md5 8d174cfa,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18186/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18186/classes-before
