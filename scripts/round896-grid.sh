#!/usr/bin/env bash
# (WARM.23) round 896 — the 8-profile output grid, CROSS-ROUND.
#
# Neither of this round's two changes has an in-binary arm (a container swap and
# a probe restructuring have nothing to switch), so the "before" side is round
# 895's committed captures in build/bench/round895-grid/*.on.txt, produced by the
...
OUT=build/bench/round896-grid
BEFORE=build/bench/round895-grid
mkdir -p "$OUT"
...
for proj in "${profiles[@]}"; do
  ...
  if [[ ! -s "$BEFORE/$name.on.txt" ]]; then
    echo "REFUSED $name: no round-895 before-capture"; status=1; continue
  fi
  added=$(comm -13 "$BEFORE/$name.on.txt" "$OUT/$name.after.txt" | wc -l)
  removed=$(comm -23 "$BEFORE/$name.on.txt" "$OUT/$name.after.txt" | wc -l)
