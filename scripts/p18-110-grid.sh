#!/usr/bin/env bash
# (LEGACY.1) step (j1) — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The change deletes TS1250 and TS18028, two diagnostics tsgo never
# emits and that fire here only below ES2015; every profile is es2020+, so neither half can move —
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-110-orch; mkdir -p "$OUT/grid"
...
# (P18.110) touches the Checker (maybe others); refuse only when NONE moved.
same=1
