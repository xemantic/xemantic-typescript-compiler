#!/usr/bin/env bash
# (LEGACY.1) step (c) — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The change deletes the `alwaysStrict: false` arms (a value
# TypeScript 7 no longer honours). All eight profiles set `alwaysStrict: true` and are all-module,
# so neither the `--listAll` half nor the `"use strict"` prologue in the EMIT half can move; a
# difference here is a FINDING. The corpus screens (both channels) and the suite are the gates.
#
# REFUSES when the two arms' Checker.class AND Emitter.class are byte-identical (a
# self-comparison prints a clean bill of health — CLAUDE.md rounds 853/946), when a capture is
# empty or TRUNCATED, and below 8 profiles.
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-103-orch; mkdir -p "$OUT/grid"
...
# (P18.103) touches the Checker and Emitter; refuse only when NEITHER moved.
same=1
for cls in Checker Emitter; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' Checker and Emitter classes are both byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
