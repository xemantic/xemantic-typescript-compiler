#!/usr/bin/env bash
# (CHK.25) round 948 — the 8-profile BEFORE/AFTER binary grid for the
# `using` / `await using` declaration form (parser + checker + emitter).
#
...
# is ADDITIVE: they carry no `using` / `await using` DECLARATION at all (measured:
# 0 hits for `^\s*(using|await using) <ident>` across all eight), while carrying
# 336 occurrences of `using` as an ordinary identifier / property name — which is
# exactly the population the contextual-keyword lookahead has to leave alone, so
# every one of the eight profiles must be byte-identical between the arms.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/round948/grid
mkdir -p "$OUT"

BEFORE=build/bench/round948/classes-before
AFTER=build/bench/round948/classes-after
