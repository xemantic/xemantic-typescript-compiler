#!/usr/bin/env bash
# (CHK.127) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk127/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk127/classes-before
AFTER=build/bench/chk127/classes-after
