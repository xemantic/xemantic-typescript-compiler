#!/usr/bin/env bash
# (WARM.28) round 901 — the 8-profile output grid, CROSS-ROUND.
#
...
# itself certify an arm. The "before" side is round 898's committed captures, produced with the
...
OUT=build/bench/round901-grid
BEFORE=build/bench/round900-grid
mkdir -p "$OUT"
