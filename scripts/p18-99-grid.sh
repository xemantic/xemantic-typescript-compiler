#!/usr/bin/env bash
# (LEGACY.0b) step 14 — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-99-orch; mkdir -p "$OUT/grid"
...
# (P18.99) touches the Parser and/or the Checker; refuse only when NEITHER moved.
same=1
for cls in Checker Parser; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' Checker.class AND Parser.class are byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
