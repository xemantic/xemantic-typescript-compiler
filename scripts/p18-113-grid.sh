#!/usr/bin/env bash
# (LEGACY.1) step (j4) — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The change changes the TARGET option surface (the es5 lib set, ES3 after TS6046), which tsgo never
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-113-orch; mkdir -p "$OUT/grid"
...
# (P18.113) touches CompilerOptions, RealLibs, the Checker; refuse only when NONE moved.
same=1
for cls in Checker CompilerOptions CompilerOptionsKt Transformer; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' Checker/CompilerOptions/CompilerOptionsKt/Transformer classes are all byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
