#!/usr/bin/env bash
# (CHK.133)(b) — the 8-profile BEFORE/AFTER binary grid for the RELATION's `this` leg (tsc's
# `compareSignaturesRelated`: a source `this` other than `void` must relate to the target's).
#
...
#
# BEFORE is HEAD eba54e4f9, AFTER is the (CHK.133)(b) tree. Reach census: the leg is entered
# 2,639-3,110 times per profile and every entry is a signature related to ITSELF (the lib's
# type-parameter `this`), so this grid is a CONTROL for the leg; the pins are its gate.  Two snapshotted class
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk133b/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk133b/classes-before
AFTER=build/bench/chk133b/classes-after
