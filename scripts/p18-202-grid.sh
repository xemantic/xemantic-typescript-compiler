#!/usr/bin/env bash
# (P18.202) the 8-profile BEFORE/AFTER binary grid for (CHK.174) — a type parameter constrained to a primitive relates to
# that primitive at the declaration and return readers. Removal-only by design; any ADDED row is a regression.
#
# BEFORE is the orchestrator's snapshot of the (P18.201) landed binary (Checker.class md5 af83e318,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18202/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18202/classes-before
