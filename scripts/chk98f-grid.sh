#!/usr/bin/env bash
# (CHK.98) stage 2 — the 8-profile BEFORE/AFTER binary grid.
#
# TWO BINARIES (HEAD b0236442 vs the (CHK.98) stage 2 tree), snapshotted class dirs, because
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk98f/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk98f/classes-before
AFTER=build/bench/chk98f/classes-after
