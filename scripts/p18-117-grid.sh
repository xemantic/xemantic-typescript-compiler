#!/usr/bin/env bash
# (CHK.135, re-scoped) — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# THIS TIME THE GRID IS A GATE, NOT A CONTROL. The round gives a read through a string/number INDEX
# SIGNATURE its real type instead of `any`, and the 8 profiles contain 59 such signatures — so
# (CHK.50)'s law applies directly: every newly real read can surface a gap `any` was hiding. A
# non-zero `added` here is the round's primary refusal signal, not noise.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-117-orch; mkdir -p "$OUT/grid"
...
# (P18.117) touches the Checker and maybe MemberResolver/Relater; refuse only when NONE moved.
same=1
