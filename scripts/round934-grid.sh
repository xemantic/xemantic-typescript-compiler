#!/usr/bin/env bash
# Round 934 — the 8-profile `--listAll` output grid for the computed-key EXCESS-PROPERTY fix.
#
# TWO BINARIES, not two flags. This change has no in-binary arm (it is one `when`
# lookup predicate plus a target-side index guard), so the grid rebuilds the PRE-934
...
# (round 805).
# Usage: scripts/round934-grid.sh <before-Checker.kt> <after-Checker.kt>
set -uo pipefail
cd /home/claude/git/xemantic-typescript-compiler

BEFORE_SRC="${1:?usage: round934-grid.sh <before-Checker.kt> <after-Checker.kt>}"
AFTER_SRC="${2:?usage: round934-grid.sh <before-Checker.kt> <after-Checker.kt>}"
CK=xemantic-typescript-compiler-core/src/commonMain/kotlin/Checker.kt
OUT=build/bench/round934-grid
