#!/usr/bin/env bash
# (CHK.73)(A)+(B) — the 8-profile BEFORE/AFTER grid for the module-object missing-member
# diagnostic and the `typeof import("…")` display.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk73a; mkdir -p "$OUT/grid"
