#!/usr/bin/env bash
# (CHK.97) stage 3 DELIVERABLE D2b — the 8-profile BEFORE/AFTER binary grid.
#
# BEFORE is HEAD d622093a7 (D2 landed), AFTER is the D2b tree.  EXPECT A CONTROL,
# NOT A GATE, and this round MEASURED why rather than repeating (P18.72)'s claim:
# an instrumented build printing every union callee whose signature COMBINATION
# refused reads ZERO on all eight profiles AND zero on cronstrue and marked.  So
# `added=0 removed=0` here proves INERTNESS and nothing about coverage
# ((CHK.124)'s lesson).  The gate for D2b is the CORPUS plus the pins.
#
# TWO BINARIES (D2 HEAD vs the D2b tree), snapshotted class dirs, because
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk97f/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk97f/classes-before
AFTER=build/bench/chk97f/classes-after
