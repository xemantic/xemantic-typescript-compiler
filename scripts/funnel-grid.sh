#!/usr/bin/env bash
# (LEGACY.0b) F6a — the 8-profile BEFORE/AFTER binary grid for the missing-property
# HEAD SUPPRESSION funnel (`RelationHeadSuppression`).
#
# IT IS A CONTROL, NOT A GATE, AND THAT WAS MEASURED RATHER THAN ASSUMED.  The rule
# rewrites a relation-error head, so the assignability path the eight profiles exercise
# constantly LOOKS like a real gate — but the message it keys on appears in NONE of the
# sixteen captures: every row on all eight profiles is `Cannot find name …` / TS2591 /
# TS2584 ((PARITY.1)), and a `grep "is missing"` over the raw captures finds nothing on
# either arm.  So a green grid here says the rule did not reach anything on tsc's own
# sources; what gates this family is the CORPUS — 2,955 active `.errors.txt` subtests, of
# which 119 carry the message and 25 carried a head the rule can suppress.
# CLAUDE.md (CHK.124): count whether the changed path fires before banking a green grid.
#
# BEFORE is HEAD cb9ff87d, AFTER is the F6a tree.  Two snapshotted class dirs, because
# the change is not switchable at run time.  REFUSES when the arms' Checker.class are
# byte-identical (an accidental self-comparison prints added=0 removed=0 on all eight and
# reads exactly like a clean bill of health — CLAUDE.md 853/946), when a capture is empty
# or TRUNCATED (`and N more error(s)` — round 811), and below 8 profiles (the pre-895
# `tsc-project-*` glob matched exactly ONE).
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/funnel/grid; mkdir -p "$OUT"
BEFORE=build/bench/funnel/classes-before
AFTER=build/bench/funnel/classes-after
