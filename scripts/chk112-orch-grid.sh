#!/usr/bin/env bash
# (CHK.108) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk112-orch/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk112-orch/classes-before
AFTER=build/bench/chk112-orch/classes-after
