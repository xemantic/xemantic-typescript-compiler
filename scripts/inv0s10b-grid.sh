#!/usr/bin/env bash
# (INV.0) step 10b — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/inv0s10b/grid; mkdir -p "$OUT"
BEFORE=build/bench/inv0s10b/classes-before
AFTER=build/bench/inv0s10b/classes-after
