#!/usr/bin/env bash
# (LEGACY.0b) step 19 — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The round changes the duplicate-identifier RELATED-span selection
# (TS6203 vs TS6204) and adds a related TS2751; all 46/94 profile rows are `Cannot find name`, so
# neither half can move — a difference here is a FINDING. The corpus ERRORS screen is the gate: 54
# active TS6203 rows and 7 TS6204 rows carry the codes.
#
# REFUSES when the two arms' Checker, Binder, TypeScriptCompiler and Relater classes are ALL
# byte-identical (a self-comparison prints a clean bill of health — CLAUDE.md rounds 853/946), when a
# capture is empty or TRUNCATED, and below 8 profiles.
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-116-orch; mkdir -p "$OUT/grid"
...
# (P18.116) touches the Checker and maybe the Binder/Relater; refuse only when NONE moved.
same=1
for cls in Checker Binder TypeScriptCompiler Relater; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' Checker/Binder/TypeScriptCompiler/Relater classes are all byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
