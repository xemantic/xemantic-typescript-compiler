#!/usr/bin/env bash
# (CHK.82) the 8-profile BEFORE/AFTER binary grid — HEAD (c0819a96) vs the
# (CHK.82) working tree. Two snapshotted class directories (the change is not
# switchable at run time); REFUSES a self-comparison, an empty or truncated
# capture, and fewer than 8 profiles.
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk82/grid
mkdir -p "$OUT"
BEFORE=build/bench/chk82/classes-before
AFTER=build/bench/chk82/classes-after
...
javap -p -cp "$AFTER" com.xemantic.typescript.compiler.Checker 2>/dev/null | grep -q augmentationTargetFile || {
  echo "REFUSED: the AFTER arm does not hold the (CHK.82) code"; exit 1; }
javap -p -cp "$BEFORE" com.xemantic.typescript.compiler.Checker 2>/dev/null | grep -q augmentationTargetFile && {
  echo "REFUSED: the BEFORE arm already holds the (CHK.82) code"; exit 1; }
shopt -s nullglob
profiles=()
