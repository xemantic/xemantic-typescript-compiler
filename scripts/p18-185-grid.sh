#!/usr/bin/env bash
# (P18.185) the 8-profile BEFORE/AFTER binary grid for (CHK.154) — a trailing `void`-accepting parameter
# is optional in signature relation (part (a) only; (b), the derived-class constructor, is its own round).
# Removal-only by design; any ADDED row is a regression.
#
...
#
# BEFORE is the orchestrator's snapshot of the (P18.184) landed binary (Checker.class md5 a716bda6,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18185/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18185/classes-before
