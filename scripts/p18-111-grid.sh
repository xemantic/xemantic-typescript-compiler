#!/usr/bin/env bash
# (LEGACY.1) step (j2) — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The change deletes the remaining `< ES2015` checker gates, which tsgo never
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-111-orch; mkdir -p "$OUT/grid"
...
# (P18.111) touches the Checker (maybe others); refuse only when NONE moved.
same=1
