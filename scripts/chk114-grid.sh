#!/usr/bin/env bash
# (CHK.114) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk114-sub/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk114-sub/classes-before
AFTER=build/bench/chk114-sub/classes-after
