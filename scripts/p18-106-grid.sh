#!/usr/bin/env bash
# (LEGACY.1) step (e) — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The change replaces the tsc-6 "classic" module-resolution
# derivation by tsgo's (unset/classic/node10 -> Node16 / NodeNext / Bundler by module kind);
# every profile is `module: NodeNext`, which derives NodeNext under both rules, so neither half
# can move — a difference here is a FINDING. The corpus screens (both channels) and the
# `-project` pins are the gates.
#
# REFUSES when the two arms' Checker, TypeScriptCompiler, ModuleResolver and CompilerOptionsKt
# classes are ALL byte-identical (a self-comparison prints a clean bill of health — CLAUDE.md
# rounds 853/946), when a capture is empty or TRUNCATED, and below 8 profiles.
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-106-orch; mkdir -p "$OUT/grid"
...
# (P18.106) touches the Checker, TypeScriptCompiler, ModuleResolver and CompilerOptions; refuse only when NONE moved.
same=1
for cls in Checker TypeScriptCompiler ModuleResolver CompilerOptionsKt; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' Checker/TypeScriptCompiler/ModuleResolver/CompilerOptionsKt classes are all byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
