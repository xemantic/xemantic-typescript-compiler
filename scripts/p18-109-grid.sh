#!/usr/bin/env bash
# (LEGACY.1) step (i) — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The change deletes `downlevelIteration`'s behaviour (TS2802 and
# the downlevel iteration arms, reachable only below ES2015); every profile is es2020+, so neither
# half can move — a difference here is a FINDING. The corpus screens and the pins are the gates.
#
# REFUSES when the two arms' Checker, Transformer, TypeScriptCompiler and Emitter classes are ALL
# byte-identical (a self-comparison prints a clean bill of health — CLAUDE.md rounds 853/946), when
# a capture is empty or TRUNCATED, and below 8 profiles.
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-109-orch; mkdir -p "$OUT/grid"
...
# (P18.109) touches the Checker (maybe Transformer/TypeScriptCompiler/Emitter); refuse only when NONE moved.
same=1
for cls in Checker Transformer TypeScriptCompiler Emitter; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' Checker/Transformer/TypeScriptCompiler/Emitter classes are all byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
