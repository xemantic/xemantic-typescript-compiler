#!/usr/bin/env bash
# (LEGACY.1) step (d2) — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The change removes the `esModuleInterop: false` /
# `allowSyntheticDefaultImports: false` arms and derives the synthetic-default default as tsgo
# does; every profile is `module: NodeNext`, where the interop is derived true whatever the flags
# say, so neither half can move — a difference here is a FINDING. The corpus screens (both
# channels) and the `-project` pins are the gates.
#
# REFUSES when the two arms' Checker, Transformer, NameResolver and CompilerOptionsKt classes are
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-105-orch; mkdir -p "$OUT/grid"
...
# (P18.105) touches the Checker, Transformer, NameResolver and CompilerOptions; refuse only when NONE moved.
same=1
for cls in Checker Transformer NameResolver CompilerOptionsKt; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' Checker/Transformer/NameResolver/CompilerOptionsKt classes are all byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
