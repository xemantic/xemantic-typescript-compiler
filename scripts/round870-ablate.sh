#!/usr/bin/env bash
# (WARM.17) round 870 — SINGLE-MISTAKE ablation of `ModuleSymbolScanIndexTest`.
#
...
apply() {
  python3 scripts/round870_ablate_apply.py "$1"
}
...
for ARM in "${ARMS[@]}"; do
  ...
  ./gradlew :xemantic-typescript-compiler-core:jvmTest --tests '*ModuleSymbolScanIndexTest*' \
      > "$OUT/$ARM.log" 2>&1
  RC=$?
...
for p in glob.glob('*/build/test-results/jvmTest/*.xml'):
    ...
    if 'ModuleSymbolScanIndex' not in r.get('name', ''):
        continue
