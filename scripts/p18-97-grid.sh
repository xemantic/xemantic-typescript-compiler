#!/usr/bin/env bash
# (LEGACY.0b) step 12 — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-97-orch; mkdir -p "$OUT/grid"
...
# (P18.97) touches BOTH the Transformer and the Emitter; refuse only when NEITHER moved.
same=1
for cls in Transformer Emitter; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' Transformer.class AND Emitter.class are byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
