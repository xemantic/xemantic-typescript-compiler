#!/usr/bin/env bash
# (CHK.112) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk112-sub/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk112-sub/classes-before
AFTER=build/bench/chk112-sub/classes-after
