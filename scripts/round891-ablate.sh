#!/usr/bin/env bash
# (WARM.18) round 891 — SINGLE-MISTAKE ablation of `VarScopeStackTest`.
#
...
apply() {
  python3 scripts/round891_ablate_apply.py "$1"
}
...
for ARM in "${ARMS[@]}"; do
  ...
  ./gradlew :xemantic-typescript-compiler-core:jvmTest --tests '*VarScopeStackTest*' \
      > "$OUT/$ARM.log" 2>&1
  RC=$?
...
for p in glob.glob('*/build/test-results/jvmTest/*.xml'):
    ...
    if 'VarScopeStack' not in r.get('name', ''):
        continue
