#!/usr/bin/env bash
# (LEGACY.0b) step 11 — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-96; mkdir -p "$OUT/grid"
...
a="$(sha256sum "$BEFORE/com/xemantic/typescript/compiler/Transformer.class" | cut -d' ' -f1)"
b="$(sha256sum "$AFTER/com/xemantic/typescript/compiler/Transformer.class" | cut -d' ' -f1)"
[[ "$a" == "$b" ]] && { echo "REFUSED: the two arms' Transformer.class are byte-identical"; exit 1; }
echo "before Transformer.class $a"
echo "after  Transformer.class $b"
shopt -s nullglob
profiles=()
