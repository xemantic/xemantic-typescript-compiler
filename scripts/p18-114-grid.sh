#!/usr/bin/env bash
# (LEGACY.0b) step 17 — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The change is the JS CommonJS export-assignment model and the 8
# profiles carry NO `.js` file at all, so neither half can move — a difference here is a FINDING.
# The corpus ERRORS screen is this round's real gate (~19 active baselines contain
# `module.exports =` or `exports.x =`), not the grid.
#
# REFUSES when the two arms' Checker, Binder, CompilerOptions and Transformer classes are ALL
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-114-orch; mkdir -p "$OUT/grid"
...
# (P18.114) touches the Checker and maybe the Binder; refuse only when NONE moved.
same=1
for cls in Checker Binder CompilerOptions Transformer; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' Checker/Binder/CompilerOptions/Transformer classes are all byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
