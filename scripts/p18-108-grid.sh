#!/usr/bin/env bash
# (LEGACY.1) step (h) — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The change deletes `outFile`-only ordering and gating; no profile
# sets `outFile`, so neither half can move — a difference here is a FINDING. The corpus screens are
# controls too (zero active subtests reach `outFile`); the hand-written pins are the gate.
#
# REFUSES when the two arms' TypeScriptCompiler, Checker, Transformer and Emitter classes are ALL
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-108-orch; mkdir -p "$OUT/grid"
...
# (P18.108) touches TypeScriptCompiler (maybe Checker/Transformer/Emitter); refuse only when NONE moved.
same=1
for cls in TypeScriptCompiler Checker Transformer Emitter; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' TypeScriptCompiler/Checker/Transformer/Emitter classes are all byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
