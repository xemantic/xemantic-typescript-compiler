#!/usr/bin/env bash
# (P18.131) the JavaScript expando MEMBER model (J1/J2) + the constructor-context display (J4) —
# the 8-profile BEFORE/AFTER grid, BOTH modes.
#
# THE GRID IS A REAL GATE THIS ROUND, unlike (P18.130)'s, and the reason is J4: the
# constructor-context type-parameter rendering is a TYPESCRIPT-visible change — a read of a
# missing member inside a constructor body now prints `Gen<T, V>` where it printed `Gen` — so
# it can move a row on profiles that are entirely `.ts`. J1/J2 remain controls here (they
# short-circuit on `isJsLikeFileName`, and no profile file is JavaScript), which is exactly the
# split to state: a green grid grades J4 and controls J1/J2.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-131-orch; mkdir -p "$OUT/grid"
