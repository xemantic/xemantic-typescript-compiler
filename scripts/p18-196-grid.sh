#!/usr/bin/env bash
# (P18.196) the 8-profile BEFORE/AFTER binary grid for (CHK.164) step 1 — truthiness narrowing splits `boolean`
# into `true | false`. A CONTROL: the census's patch arm moved 0 rows on every profile; the pins are the gate.
#
# BEFORE is the orchestrator's snapshot of the (P18.195) landed binary (Checker.class md5 7a1ba8b3,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18196/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18196/classes-before
