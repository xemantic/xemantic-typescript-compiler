#!/usr/bin/env bash
# (P18.194) the 8-profile BEFORE/AFTER binary grid for (CHK.168) round 1 — an annotated arrow's EXPRESSION body
# is return-checked through the block-body return path. The census's forced arm read +0 on every profile
# (21-65 annotated expression-bodied arrows reached per profile) — a weak but real gate.
#
...
#
# BEFORE is the orchestrator's snapshot of the (P18.193) landed binary (Checker.class md5 661591c5,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18194/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18194/classes-before
