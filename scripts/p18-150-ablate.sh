#!/usr/bin/env bash
# (P18.150) ablation — ONE injected mistake per arm (round 807), each reverted before the next.
#
...
SNAP=build/bench/p18-150-ablate; mkdir -p "$SNAP"
TESTS="--tests *ClassImplementsChainDepthTest* --tests *InterfaceExtendsChainAndShapesTest*"
...
case "${1:-all}" in
a1|all)
  # a1 — the PROPERTY-pair drill OFF: the chain stops at the whole-object line again.
  run_arm a1 patch \
    'getPropertyElaborationChain(derivedType, basePropType)
                                        ?.let { deeper -> chain.addAll(deeper.map { "  $it" }) }' \
    'if (false) getPropertyElaborationChain(derivedType, basePropType)
                                        ?.let { deeper -> chain.addAll(deeper.map { "  $it" }) }' ;;&
a2|all)
  # a2 — the RETURN-pair drill OFF.
  run_arm a2 patch \
    'getPropertyElaborationChain(sourceReturn, targetReturn)
                    ?.let { deeper -> chain.addAll(deeper.map { "    $it" }) }' \
    'if (false) getPropertyElaborationChain(sourceReturn, targetReturn)
                    ?.let { deeper -> chain.addAll(deeper.map { "    $it" }) }' ;;&
a3|all)
  # a3 — the double-append GUARD off: the property drill runs even when
  # `addSignatureElaboration` already elaborated, so a parameter mismatch grows a
  # contradictory tail. This is the guard the parameter-mismatch control pins.
  run_arm a3 patch \
