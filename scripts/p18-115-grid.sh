#!/usr/bin/env bash
# (LEGACY.0b) step 18 — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The round's three mechanisms are self-name resolution under an
# ambiguous project root, an `isolatedDeclarations` code/anchor, and a signature-rendering quote
# style; no profile sets `isolatedDeclarations`, none is a self-referencing package, and all 46/94
# profile rows are `Cannot find name` (PARITY.1 — the grid is structurally blind to display). A
# difference here is a FINDING. The corpus ERRORS screen is the gate.
#
# REFUSES when the two arms' Checker, ModuleResolver, NameResolver and TypeScriptCompiler classes are
# ALL byte-identical (a self-comparison prints a clean bill of health — CLAUDE.md rounds 853/946),
# when a capture is empty or TRUNCATED, and below 8 profiles.
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-115-orch; mkdir -p "$OUT/grid"
...
# (P18.115) touches the Checker, the resolvers and maybe TypeScriptCompiler; refuse only when NONE moved.
same=1
for cls in Checker ModuleResolver NameResolver TypeScriptCompiler; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' Checker/ModuleResolver/NameResolver/TypeScriptCompiler classes are all byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
