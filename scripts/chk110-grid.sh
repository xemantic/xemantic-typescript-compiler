#!/usr/bin/env bash
# (CHK.110) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk110-sub/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk110-sub/classes-before
AFTER=build/bench/chk110-sub/classes-after
