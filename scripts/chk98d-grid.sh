#!/usr/bin/env bash
# (CHK.98)(d) — the 8-profile BEFORE/AFTER binary grid.
#
# TWO BINARIES (HEAD 6cb5395d vs the (CHK.98)(d) tree), snapshotted class dirs, because
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk98d/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk98d/classes-before
AFTER=build/bench/chk98d/classes-after
