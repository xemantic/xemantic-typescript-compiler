#!/usr/bin/env bash
# (CHK.96) stage 2 — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk96s2/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk96s2/classes-before
AFTER=build/bench/chk96s2/classes-after
