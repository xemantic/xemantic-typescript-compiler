#!/usr/bin/env bash
# (CHK.113) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk113-sub/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk113-sub/classes-before
AFTER=build/bench/chk113-sub/classes-after
