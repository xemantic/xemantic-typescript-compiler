#!/usr/bin/env bash
# (WARM.16) round 869 — the 8 dashboard profiles captured from TWO class
...
# `AnnScopeStack` (a class that did not exist before this round) and the BEFORE dir
...
#
#   usage: scripts/round869-grid.sh <before-class-dir> <after-class-dir> [outdir]
set -uo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
BEFORE="${1:?before class dir}"
AFTER="${2:?after class dir}"
OUT="${3:-build/bench/r869-grid}"
mkdir -p "$OUT"
...
javap -p -cp "$AFTER" com.xemantic.typescript.compiler.AnnScopeStack >/dev/null 2>&1 || {
  echo "REFUSED: the AFTER dir has no AnnScopeStack — it predates (WARM.16)" >&2; exit 1; }
if javap -p -cp "$BEFORE" com.xemantic.typescript.compiler.AnnScopeStack >/dev/null 2>&1; then
  echo "REFUSED: the BEFORE dir HAS AnnScopeStack — the two arms are the same build" >&2; exit 1
fi
