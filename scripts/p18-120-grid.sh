#!/usr/bin/env bash
# (P18.120) a `for`-header / `for-in` binding's type — the 8-profile BEFORE/AFTER grid, BOTH modes.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-120-orch; mkdir -p "$OUT/grid"
...
# (P18.120) touches the Checker; refuse only when NONE of the four moved.
same=1
