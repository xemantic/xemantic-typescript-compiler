#!/usr/bin/env bash
# (CHK.109) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk109-sub/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk109-sub/classes-before
AFTER=build/bench/chk109-sub/classes-after
