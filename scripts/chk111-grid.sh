#!/usr/bin/env bash
# (CHK.111) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk111-sub/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk111-sub/classes-before
AFTER=build/bench/chk111-sub/classes-after
