#!/usr/bin/env bash
# (P18.140) — the 8-profile BEFORE/AFTER grid for two ORDER-model rows: the union member order
# in `resolveSimpleTypeName`'s legacy STRING path, and the constituent order in the B219
# `NoInfer` excess-property walker.
#
# THIS IS A **CONTROL**, AND (PARITY.1) IS THE WHOLE REASON: the grid compares diagnostic ROWS,
# and all 46 rows of seven profiles plus all 94 of `harness` are `Cannot find name ...`, which
# renders no union and no type at all. A type-DISPLAY change cannot move a single one of them.
# The `full` arm (from (P18.135)) widens that to the whole capture including chain lines, which
# is why it is worth running here even though the head rows cannot move — but the profiles carry
# no chain line either, so both arms read 0 by construction.
#
# THE GATE IS THE CORPUS SCREEN, both channels, where these two rows are now ACTIVE in the
# compared count (3,069 -> 3,071 with `@Ignore`d 52 -> 50) — that IS the closure receipt. This
# family is the highest-blast-radius one left, because thousands of green baselines render a
# union, so the screen against a throwaway build was the instrument that decided the shape.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-140-orch; mkdir -p "$OUT/grid"
