...
# The Gradle module holding the compiler. Task invocations must be QUALIFIED
# with it: an unqualified name now runs in all three projects.
...
def resolve_classpath():
    ...
    with open(init_script, "w") as f:
        f.write(
            ...
            "// Registered ONLY in the compiler module. Under `allprojects` this\n"
            "// would also hit the root (no jvmRuntimeClasspath — the task throws)\n"
            "// and -api (which HAS one, and would print a second, wrong classpath\n"
            "// that the reader below could pick instead).\n"
            "rootProject {\n"
            "    // findProject, not project(): an init script applies to EVERY\n"
            "    // build in the invocation, and `build-logic` has no such project.\n"
            '    findProject(":%s")?.tasks?.register("xtscPrintJvmRuntimeClasspath") {\n' % MODULE
            + "        doLast {\n"
    ...
    proc = subprocess.Popen(
        [os.path.join(REPO, "gradlew"), "--console=plain", "-I", init_script,
         ":%s:compileKotlinJvm" % MODULE,
         ":%s:xtscPrintJvmRuntimeClasspath" % MODULE],
