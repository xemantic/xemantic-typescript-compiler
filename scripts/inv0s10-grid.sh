#!/usr/bin/env bash
# (INV.0) step 10 — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/inv0s10/grid; mkdir -p "$OUT"
BEFORE=build/bench/inv0s10/classes-before
AFTER=build/bench/inv0s10/classes-after
