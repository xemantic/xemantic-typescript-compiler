#!/usr/bin/env bash
# (CHK.92) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk92-sub/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk92-sub/classes-before
AFTER=build/bench/chk92-sub/classes-after
