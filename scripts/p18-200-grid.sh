#!/usr/bin/env bash
# (P18.200) the 8-profile BEFORE/AFTER binary grid for (CHK.169) — a class declared in a MODULE file is resolved
# through the file's own symbol (and its base through the per-file view) at the argument reader, so `this` is typed.
# A REAL GATE: the fix picks a different class symbol 7-91 times per profile; census predicted +0 after (CHK.170).
#
# BEFORE is the orchestrator's snapshot of the (P18.199) landed binary (Checker.class md5 da71308e,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18200/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18200/classes-before
