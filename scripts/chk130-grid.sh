#!/usr/bin/env bash
# (CHK.130) — the 8-profile BEFORE/AFTER binary grid for the union-member
# parenthesization fix.
#
...
#
# BEFORE is HEAD df592f2e9, AFTER is the (CHK.130) tree.  Two snapshotted class
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk130/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk130/classes-before
AFTER=build/bench/chk130/classes-after
