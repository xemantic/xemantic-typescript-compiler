#!/usr/bin/env bash
# (LEGACY.0b) step 13 — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-98-orch; mkdir -p "$OUT/grid"
...
# (P18.98) touches the Checker only.
same=1
for cls in Checker; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' Checker.class is byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
