#!/usr/bin/env bash
# (P18.203) the 8-profile BEFORE/AFTER binary grid for (CHK.172) — `emitDeclarationOnly` runs the normal checker (only
# emit is restricted) and `noCheck` is honoured. The standard grid is `--noEmit`, which does not take the
# declarationOnly path, so this grid is a CONTROL for the plain path; the edo arm is measured separately.
#
# BEFORE is the orchestrator's snapshot of the (P18.202) landed binary (Checker.class md5 749a21d5,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18203/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18203/classes-before
