#!/usr/bin/env bash
# (CHK.115) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk115-sub/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk115-sub/classes-before
AFTER=build/bench/chk115-sub/classes-after
