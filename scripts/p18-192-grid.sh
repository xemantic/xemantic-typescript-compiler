#!/usr/bin/env bash
# (P18.192) the 8-profile BEFORE/AFTER binary grid for (CHK.167) round 1 — a non-nullish UNION source is
# related to an object-family target at the declaration, assignment and property-assignment readers. THIS IS
# A REAL GATE: the census's naive opening added 28-35 false-positive rows per profile; round 1's shape was
# predicted +0.
#
# BEFORE is the orchestrator's snapshot of the (P18.191) landed binary (Checker.class md5 7769c37a,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18192/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18192/classes-before
