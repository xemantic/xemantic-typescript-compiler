#!/usr/bin/env bash
# (P18.188) the 8-profile BEFORE/AFTER binary grid for (CHK.156) — literal narrowing splits `boolean`
# into `true | false` (so `=== true` / `=== false` remove each half). tsc's sources compare booleans
# with literals often; display text may move; every added row must be a tsgo row.
#
# BEFORE is the orchestrator's snapshot of the (P18.187) landed binary (Checker.class md5 22fd3944,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18188/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18188/classes-before
