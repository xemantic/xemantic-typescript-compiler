#!/usr/bin/env bash
# (CHK.122) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk122/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk122/classes-before
AFTER=build/bench/chk122/classes-after
