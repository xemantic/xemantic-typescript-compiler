#!/usr/bin/env bash
# (P18.183) the 8-profile BEFORE/AFTER binary grid for (CHK.152) step 1 — a named-object argument is
# RELATED to a named-object parameter. THIS GRID IS A REAL FALSE-POSITIVE GATE: the census measured
# 7,494 narrow-scope argument pairs on tsc-project and 14,311 on harness reaching the opened gate,
# predicted to add 0 rows; any added row must be a tsgo row.
#
...
#
# BEFORE is the orchestrator's snapshot of the (P18.182) landed binary (Checker.class md5 427bcc1c,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18183/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18183/classes-before
