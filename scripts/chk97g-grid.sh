#!/usr/bin/env bash
# (CHK.97) D3 — the 8-profile BEFORE/AFTER binary grid for the IDENTICAL-signature
# union contextual signature.
#
...
#
# BEFORE is HEAD e817f9765, AFTER is the (CHK.97) D3 tree.  Two snapshotted class
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk97g/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk97g/classes-before
AFTER=build/bench/chk97g/classes-after
