#!/usr/bin/env bash
# (LEGACY.1) step (f) — the 8-profile BEFORE/AFTER grid, in BOTH modes.
#
# EXPECT CONTROLS, AND COUNT THEM. The change routes `module: amd/umd/system` onto the CommonJS
# transform and deletes the arms only those kinds reached; every profile is `module: NodeNext`,
# so neither half can move — a difference here is a FINDING. The corpus screens (the EMIT channel
# above all) and the `-project`/`-externals` pins are the gates.
#
# REFUSES when the two arms' Checker, Transformer, TypeScriptCompiler and CompilerOptionsKt
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18-107-orch; mkdir -p "$OUT/grid"
...
# (P18.107) touches the Checker, Transformer, TypeScriptCompiler and CompilerOptions; refuse only when NONE moved.
same=1
for cls in Checker Transformer TypeScriptCompiler CompilerOptionsKt; do
...
[[ "$same" == 1 ]] && { echo "REFUSED: the two arms' Checker/Transformer/TypeScriptCompiler/CompilerOptionsKt classes are all byte-identical"; exit 1; }
shopt -s nullglob
profiles=()
