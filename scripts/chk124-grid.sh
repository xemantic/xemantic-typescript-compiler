#!/usr/bin/env bash
# (CHK.124) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk124/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk124/classes-before
AFTER=build/bench/chk124/classes-after
