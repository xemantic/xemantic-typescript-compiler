#!/usr/bin/env bash
# (P18.149) ablation — ONE injected mistake per arm (round 807), each reverted before the next.
#
...
SNAP=build/bench/p18-149-ablate; mkdir -p "$SNAP"
TESTS="--tests *InterfaceExtendsChainAndShapesTest* --tests *ReturnedByChainCollapseTest*"
...
case "${1:-all}" in
a1|all)
  # a1 — chain routing OFF: the engine chain is never adopted.
  run_arm a1 patch \
    ...
    'if (chain.isNullOrEmpty()) return null
        return null' ;;&
a2|all)
  # a2 — structural arm OFF: the post-loop fallback never runs.
  run_arm a2 patch \
    ...
    'if (false && tryEmitTS2430Structural(
                        ifaceDecl, derivedName, baseName, baseType, baseProps,
                        derivedProps, derivedMethods, derivedIfaceType, source, fileName,
                    )
                ) return' ;;&
a3|all)
  # a3 — PER-MEMBER verdict instead of the whole-type relation (the bivariance defect).
  # Deleting the whole-type guard IS that mistake faithfully: what then decides the arm is
  # `getPropertyElaborationChain`'s own per-member `checkTypeRelatedTo`, i.e. exactly the
  # comparison that loses `bivariantParams`.
  run_arm a3 patch \
    ...
    'val chain = getPropertyElaborationChain(derivedIfaceType, baseType)' ;;&
a4|all)
  # a4 — CONFINEMENT off: the structural arm may re-decide a member the loop ruled on.
  run_arm a4 patch \
    'if (!isMissingShape) return false' \
    'if (false && !isMissingShape) return false' ;;
esac
