#!/usr/bin/env bash
# (CHK.98) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk101-orch/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk101-orch/classes-before
AFTER=build/bench/chk101-orch/classes-after
