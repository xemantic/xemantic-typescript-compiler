#!/usr/bin/env bash
# (CHK.33) the 8-profile BEFORE/AFTER binary grid for the union-callee ARITY recovery
# (`Checker.signatureDeclaredArity` shared with round 446's property-access reader, plus the
# TS2555 arm for a rest-bearing combined signature).
#
# EXPECT A GATE, NOT A CONTROL — but say so with a COUNT, not an argument.  tsc's own
# sources call methods through union receivers; whether any of those members declares a
# BINDING-PATTERN parameter is the question this grid answers, and (CHK.124)'s law says a
# green grid is a statement about the corpus until that count is taken.  The REAL gate for
# this round is `marked` (18 -> 10 expected: the 8 TS2554 rows) plus the pins.
#
# BEFORE is HEAD f9caa002a, AFTER is the (CHK.33) tree.  Two snapshotted class dirs.
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk33/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk33/classes-before
AFTER=build/bench/chk33/classes-after
