#!/usr/bin/env bash
# (P18.187) the 8-profile BEFORE/AFTER binary grid for (CHK.155) — a captured read of an outer variable
# inside an expression-bodied arrow is not TS2454 unless the variable is never assigned (tsgo's
# `isOuterVariable && !isNeverInitialized`). Removal-only; any ADDED row is a regression.
#
# BEFORE is the orchestrator's snapshot of the (P18.186) landed binary (Checker.class md5 bd670791,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18187/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18187/classes-before
