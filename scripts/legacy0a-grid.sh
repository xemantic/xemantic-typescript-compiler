#!/usr/bin/env bash
# (LEGACY.0a) — the 8-profile BEFORE/AFTER binary grid for tsc's stable type
# ordering (`StableTypeOrdering.kt`, the union INTERNING order); labels only.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/legacy0a/grid
mkdir -p "$OUT"

BEFORE=build/bench/legacy0a/classes-before
AFTER=build/bench/legacy0a/classes-after
