#!/usr/bin/env bash
# (WARM.24) round 897 — the 8-profile output grid, CROSS-ROUND.
#
...
# committed captures in build/bench/round896-grid/*.after.txt, produced by the
...
OUT=build/bench/round897-grid
BEFORE=build/bench/round896-grid
mkdir -p "$OUT"
...
[[ -f "$MAIN/com/xemantic/typescript/compiler/NameCensus.class" ]] || {
  echo "REFUSED: class dir predates round 897 (no NameCensus)"; exit 1; }
...
for proj in "${profiles[@]}"; do
  ...
  if [[ ! -s "$BEFORE/$name.after.txt" ]]; then
    echo "REFUSED $name: no round-896 before-capture"; status=1; continue
  fi
