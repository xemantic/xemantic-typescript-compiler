#!/usr/bin/env bash
# (CHK.125) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk125/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk125/classes-before
AFTER=build/bench/chk125/classes-after
