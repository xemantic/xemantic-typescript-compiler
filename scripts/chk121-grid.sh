#!/usr/bin/env bash
# (CHK.121) — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk121/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk121/classes-before
AFTER=build/bench/chk121/classes-after
