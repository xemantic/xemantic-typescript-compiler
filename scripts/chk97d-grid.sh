#!/usr/bin/env bash
# (CHK.97) stage 3 — the 8-profile BEFORE/AFTER binary grid.
#
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/chk97d/grid; mkdir -p "$OUT"
BEFORE=build/bench/chk97d/classes-before
AFTER=build/bench/chk97d/classes-after
