#!/usr/bin/env bash
# (LEGACY.0b) step 3 — the 8-profile BEFORE/AFTER binary grid for the four families this
# round landed: F8 unused-local ANCHORS (the import name, the one-element binding element,
# the TS6199 declaration list), F6 TS2497 DELETED (TypeScript 7 references the code nowhere),
# F0 TS1127 widened to one character, and F8 TS8017 widened to the declaration.
#
# THREE OF THE FOUR ARE A CONTROL HERE, AND SAYING SO IS THE POINT.  Not one of the eight
# tsconfigs sets `noUnusedLocals`/`noUnusedParameters`, so the unused-declaration emitter
# never runs (the same control (LEGACY.0b) step 2 measured for F4); no profile source holds
# an invalid character, so TS1127 cannot fire; and every profile file is `.ts` with
# `checkJs` off, so TS8017 cannot fire.  What the grid IS a gate for is the TS2497 DELETION:
# every profile imports across files constantly, and a deletion that reached further than
# the `export =` interop shapes would print as `removed=N` on a profile.  (PARITY.1) also
# applies: every row on all eight profiles is `Cannot find name …`, so nothing here names a
# type.
#
# BEFORE is HEAD b5556610, AFTER is the (LEGACY.0b) step 3 tree.  Two snapshotted class
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/legacy0b3/grid; mkdir -p "$OUT"
BEFORE=build/bench/legacy0b3/classes-before
AFTER=build/bench/legacy0b3/classes-after
