#!/usr/bin/env bash
# (P18.190) the 8-profile BEFORE/AFTER binary grid for (CHK.158) — a type-guard predicate reached through a
# VARIABLE (`const { isArray } = Array`) narrows, read from the callee's signature. Removal-only expected.
#
# BEFORE is the orchestrator's snapshot of the (P18.189) landed binary (Checker.class md5 2ac572bb,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18190/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18190/classes-before
