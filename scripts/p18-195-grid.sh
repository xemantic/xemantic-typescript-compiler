#!/usr/bin/env bash
# (P18.195) the 8-profile BEFORE/AFTER binary grid for (CHK.166)(a) step 1 — value-aware enum truthiness in
# `narrowByTruthiness`. A CONTROL: the census's patch arm changed 88-92 narrowings per profile and moved 0
# rows (full text byte-identical); the pins are the gate.
#
...
#
# BEFORE is the orchestrator's snapshot of the (P18.194) landed binary (Checker.class md5 70f3a226,
...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=build/bench/p18195/grid; mkdir -p "$OUT"
BEFORE=build/bench/p18195/classes-before
