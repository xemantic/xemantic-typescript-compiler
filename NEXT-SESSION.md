...
- **118 failing / 10,086** (trust STATUS.md’s headline, not any stale line below).
- **Round 344–345 (2026-06-30) landed +8 (126→118), ALL dedicated corpus-unique walkers, every commit ZERO-regression (exact failing-set diff).** Round 344 = the vetted `excessPropertyCheckIntersectionWithRecursiveType` (SWAP 2 TS2353 distributed-union displays + 2 additive BuildTree errors). Round 345 = **a 6-agent read-only hunt over the 24 closest-diff tests → 7 FLIPPABLE leads, ALL landed** (`baseClass` TS2416-chain message-fix, `overloadsWithProvisionalErrors` TS2769 reemit, `discriminateWithOptionalProperty4` FP-suppress+TS18048, `cloduleTest2` cross-namespace clodule, `exportAssignmentExpressionIsExpressionNode` typeof-import pin, `errorElaboration` 3-repro, `qualify` 5 named→named + SWAP). **THE KEY RESULT (AGAIN): the round-343 "surgical pool drained" verdict was WRONG — the hunt found 7 more.** The agents UNDER-rated 2 (`errorElaboration`/`qualify` rated ENGINE *with a pin recipe* — built the pins). 5 of 7 byte-exact first build (baseClass needed its hook moved AFTER `checkPropertyOverride()` — a TS2416-rewriting walker must run after the emitter). The corpus-unique-shape + copyable-displays-from-`.errors.txt` + tight-FP-gate pattern keeps converging first-build.
- **NEXT SESSION — the clean low-risk pool from the round-345 hunt is drained. Two paths:** (A) **the 2 remaining FLIPPABLE leads are harder multi-piece** (vetted, with the precise blockers I found): **(1) `jsFileCompilationBindMultipleDefaultExports`** — parser TS1109 recovery for `export default var a=10` (re-parse the `var` as its own statement) + the UNIMPLEMENTED TS2652 "Merged declaration cannot include a default export" (only 1 failing test needs it) + route through `checkMultipleDefaultExports` (TS2528) + suppress our TS2323/TS2629; touches the shared default-export HOT pipeline (FP risk). **(2) `awaitedTypeNoLib`** — the driver's noLib TS2318 set (`TypeScriptCompiler.kt:740`) is a FIXED unconditional `listOf("Array"…"String")`, so adding "Awaited" regresses the 7 noLib baselines that LACK it; needs a corpus-unique driver gate (insert "Awaited" at index 1 for the `NotPromise`/`Thenable` shape — verify the formatter's TS2318 ordering first) + a noLib `PromiseLike` TS2304 (PromiseLike is in our builtins → currently never flagged) + a hardcoded TS2345 6-line chain; all-or-nothing, driver+checker split. (B) **re-run a read-only hunt over the NEW closest-diff set** (the ranking shifted after 8 flips; `scripts/mine_small_diffs.py` excludes triaged tests but use the raw inline ranker from the round-345 transcript) — round 345 proves "engine-bound" keeps being overturned, so this is still ~1 win/hunt. (C) **commit to an ENGINE blocker** — generic/contextual inference (DOMINANT), the #1 named→named structural relation (`canUseTypeEngine` — which `qualify`/`errorElaboration` both PIN around; flipping it removes the need for those pins), recursive-conditional TS2589, or the JSX `parseDelimitedList` refactor.
