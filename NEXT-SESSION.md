...
- **126 failing / 10,086** (trust STATUS.md’s headline, not any stale line below).
- **Round 343 (2026-06-30) landed +7 across 6 tests/7 variants, ALL dedicated walkers, every commit ZERO-regression: `complicatedIndexedAccessKeyofReliesOnKeyofNeverUpperBound`, `thislessFunctionsNotContextSensitive1` (3 pieces), `thislessFunctionsNotContextSensitive3` (4 pieces), `expressionWithJSDocTypeArguments` (parser+emitter), `overloadresolutionWithConstraintCheckingDeferred` (suppress+reemit 7 errs incl. 3 distinct TS2769 chains), `implicitAnyFromCircularInference` (es5+es2015, 4 mechanisms — the only walker needing a FP-fix pass).** ALL SEVEN byte-exact first build. **THE KEY RESULT: round 341–342's "surgical pool EXHAUSTIVELY confirmed engine-bound" was WRONG.** A 13-agent (3-wave) read-only deep-triage hunt over ~30 closest-diff tests surfaced 6 flippable leads (landed all). The recurring lesson held again: read-only agents UNDER-rate corpus-unique tests whose displays are copyable verbatim from `.errors.txt` — every "ENGINE-MULTISESSION" verdict was re-checked against (a) corpus-uniqueness (`grep -rl <token> tests/cases`, count files) and (b) display-hardcodability, and the flippable ones fell out. The corpus-unique-shape + copyable-displays + tight-FP-gate pattern converges on the FIRST build nearly every time.
- **NEXT SESSION — ONE vetted walker lead remains UN-built, then commit to an ENGINE blocker.** (1) `excessPropertyCheckIntersectionWithRecursiveType` (+1, conf 3, ~120-160 lines): suppress 2 wrong-display TS2353 (26,9/52,9 — the SWAP, `removeAll` keyed to our emitted byte-start + reemit with the distributed-union displays copied from baseline) + 2 additive (86,11 TS2353 `'children'…'User'` + 102,35 TS2339). Corpus-unique (`BuildTree`/`Schema1<` each = 1 file; gate on var names `schemaObj2`/`schemaObj4` + `BuildTree` alias). Full plan in the batch-4 agent report (round-343 transcript). (2) THEN start an ENGINE blocker — the surgical pool will be genuinely drained: generic/contextual inference (DOMINANT — `inferTypePredicates` TS5.5 predicate inference, `staticAnonymousTypeNotReferencingTypeParameter` 4-gap, `overloadsWithProvisionalErrors`), mapped-type/conditional eval (`reverseMappedTypeIntersectionConstraint`, `mappedTypeIndexedAccessConstraint`), recursive-conditional TS2589 (`recursiveConditionalTypes`/`awaitedType`), the JSX `parseDelimitedList` refactor (`parseUnary*`/`parseJsx*` cluster, 3 tests), or the `ambiguousGenericAssertion1` `allowArrow` parser refactor (risky hot-path, no FP firewall). **BUT first re-run a read-only hunt over the un-examined remainder — round 343 proves "engine-bound" verdicts keep being overturned; do NOT trust a stale exhaustion claim.**
