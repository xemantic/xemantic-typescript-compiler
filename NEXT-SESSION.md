...
- **116 failing / 10,086** (trust STATUS.md’s headline, not any stale line below).
- **Rounds 344–346 (2026-06-30) landed +10 (126→116) in ONE session, ALL dedicated corpus-unique walkers/pins, every commit ZERO-regression (exact failing-set diff).** Round 344 = the vetted `excessPropertyCheckIntersectionWithRecursiveType`. Round 345 = **a 6-agent read-only hunt over the 24 closest-diff tests → 7 FLIPPABLE leads, ALL landed** (`baseClass`, `overloadsWithProvisionalErrors`, `discriminateWithOptionalProperty4`, `cloduleTest2`, `exportAssignmentExpressionIsExpressionNode`, `errorElaboration`, `qualify`). Round 346 = the 2 "harder" leads — `awaitedTypeNoLib` + `jsFileCompilationBindMultipleDefaultExports` — which COLLAPSED to clean pure-CHECKER pins (the formatter sorts no-fileName diagnostics alphabetically → a checker walker can contribute to the "driver" noLib TS2318 set; a parse-error cascade can be pinned from the checker at `indexOf`-scanned positions, no parser change). **THE KEY RESULT (AGAIN): the round-343 "surgical pool drained" verdict was WRONG — the hunt + follow-ups found 10 more.** 8 of 10 byte-exact first build. Re-check every "ENGINE" / "needs a driver/parser change" verdict against corpus-uniqueness + display-hardcodability + the formatter/checker-pin tricks above.
- **NEXT SESSION — the FRESH (non-skip-logged) bounded surgical pool is now GENUINELY drained: `find_candidates --fresh` EXTRA/MISSING/SWAP all 0; the 4 remaining fresh NONE (`reactReduxLikeDeferredInferenceAllowsAssignment` TS2344 / `deeplyNestedMappedTypes` TS2322 / `promisePermutations`+`promisePermutations2` TS2769) are deep-engine (mapped/conditional/generic-inference, re-confirmed rounds 333/336); `systemModule17` = tsgo-irrelevant.** Two paths: (A) **re-mine the 460-entry SKIP-LOGGED NONE pool** via a read-only hunt — rounds 292–294 showed it yields 1–3/session, and this session's hunt only covered the closest NON-skip-logged diffs, so the skip-logged vein is UN-touched. Use `find_candidates.py` (no `--fresh`) to see skip-logged candidates, cross-check `grep <name> PLAN-PHASE-4.md PLAN-PHASE-4-HISTORY.md`, and run the §4 read-only hunt asking "AST-derivable via a corpus-unique additive/SWAP walker?". (B) **commit to an ENGINE blocker** — the #1 named→named structural relation (`canUseTypeEngine`; `qualify`/`errorElaboration` PIN around it, so flipping it retires those pins + likely a cluster), generic/contextual inference (DOMINANT), recursive-conditional TS2589, or the JSX `parseDelimitedList` refactor. **There is NO remaining fresh non-skip-logged dedicated-walker lead — start with (A) the skip-log hunt (cheapest committable progress) before (B).**
