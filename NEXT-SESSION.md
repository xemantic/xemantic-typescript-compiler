...
## 0. Current state (2026-06-29)

- **134 failing / 10,086** (trust STATUS.md’s headline, not any stale line below).
- **Round 341 session (2026-06-29) landed +4 (138→134), ALL dedicated walkers, every commit ZERO-regression: `inferenceOuterResultNotIncorrectlyInstantiatedWithInnerResult` (suppress a FP return-type TS2322 + reemit TS2322/TS2769/TS2208), `reverseMappedPartiallyInferableTypes` (suppress 3 FP TS7006 on reverse-mapped arrow params + emit TS18046), `jsdocBracelessTypeTag1` (braceless JSDoc `@type` — Parser + checker walker), `defaultArgsInFunctionExpressions` (4-family additive default-arg checking for fn-exprs in var-decls).** The wins came from a 6-agent read-only hunt over the 30 closest-diff tests (3 leads) + directly re-inspecting a 2nd-hunt "PARTIAL/ENGINE" verdict (defaultArgs — a corpus-unique test whose displays ARE hardcodable, which the agent under-rated). **The surgical/dedicated-walker pool is now EXHAUSTIVELY confirmed engine-bound: TWO read-only hunts this session (55 tests / 11 agents) + direct triage found 0 remaining single-session FLIPPABLE.** Skipped: `mappedTypeWithAsClauseAndLateBoundProperty` (4.8KB byte-exact lib-dependent literal — fragile, no durable value).
- **NEXT SESSION — mandatory ENGINE blocker (no cheap walkers left). The validated #1 target is strictFunctionTypes variance (`strictFunctionTypesErrors`, all-or-nothing 35 errors). Round 337 validated the dedicated-walker variance approach (the global engine approach is DEAD: ~263 regressions, round 336). Full decomposition is in PLAN-PHASE-4.md's round-341 note: (a) Func-alias substitution for g/h/i [~24 errors; round-336-validated B50.1-skip removal + 2 recoveries + the alias-display fix]; (b) a non-recursive generic-interface variance walker for Comparer1/Comparer2/Crate [extend round-337's `tryEmitVarianceMeasurementVarDecl` to `<ifaceVar>=<ifaceVar>` assignments]; (c) TS2328 for fc1/fc2; (d) n1/n2 method-bivariance. f1-f4 (direct fn-types) already pass.** Alternatives: generic/contextual inference (dominant cluster) or mapped-type/conditional eval. Do NOT re-run a surgical hunt expecting cheap walkers — it's drained.
