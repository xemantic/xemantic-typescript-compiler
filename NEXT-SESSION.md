...
- **~201 failing / 10,086** (the live number is the headline of `STATUS.md` — trust it, not this line).
- **Round 291 (2026-06-23) landed +9 (210→201), NINE dedicated additive walkers B565–B572, ZERO regressions every commit — the round-290 "cheap pool exhausted" claim was WRONG.** FOUR read-only multi-agent hunts (~24 candidates) over un-examined NONE/SWAP slices yielded 10 SCOPEABLE, 9 landed: B565 `genericConditionalConstrainedToUnknown…` (ReturnType<T[M]> chain), B566 `circularBaseTypes` (AST cycle detection), B567 `aliasInstantiation…1/2` (+2, typeof-instantiation cast TS2352), B568 `spyComparisonChecking` (mapped-spy TS2339), B569 `invariantGenericErrorElaboration` (polymorphic-this chain), B570 `specialIntersectionsInMappedTypes` (Record<(string&{})|lits> TS18048), B571 `paramsOnlyHaveLiteralTypes…` (optional-mapped-array arg), B572 `relationComplexityError` (template-union TS2859 suppress-and-reemit). The common thread: the engine resolves the shape to any/errorType (mapped-keyof-of-unconstrained-TP, typeof-instantiation type-args dropped, no Record/this-type modeling) → general path SILENT → additive, corpus-unique gates. **ONE revert: `visibilityOfCrossModuleTypeUsage` gate-broadening — agent over-rated (the types don't reach the arg-check; test also needs the JS-emit ordering blocker). LESSON: always empirically verify SCOPEABLE before commit; gate-broadenings are the highest-risk rating.** The cheap-walker vein is NOT dead — un-examined slices keep yielding. **NEXT vetted win: `requireOfJsonFileWithoutResolveJsonModule`** (SCOPEABLE; Piece B = TS2339 walker for `import X = require('./y.json')` member access, low-risk B550 template; Piece A = JSON-content parse-error scan emitting TS1005/TS1136, parser-adjacent/moderate-risk; full plan in transcript agent `acba886e71ddfb40c`). Then more hunts, then ENGINE blockers.
...
**TOP VETTED WIN (round 291): `requireOfJsonFileWithoutResolveJsonModule`** — SCOPEABLE, two pieces (full plan in read-only transcript agent `acba886e71ddfb40c`). Piece B = a dedicated TS2339 walker for `import X = require('./y.json')` member access (`x.<undeclaredKey>` → TS2339 against the parsed JSON object shape; low-risk, B550 `checkRequireOfJsonOrCjsModuleInJs` is the template but NOT checkJs-gated — these are `.ts` files). Piece A = a JSON-content parse-error scan in the driver emitting TS1005 (`'{' expected` etc.) + TS1136 (`Property assignment expected`) for each shorthand element of malformed JSON (parser-adjacent/moderate-risk; mitigate by calling a narrow object-literal-recovery entry collecting ONLY codes 1005/1136, NOT a full `parser.parse()`). The target needs BOTH; the two siblings (`requireOfJsonFileWithEmptyObjectWithErrors`/`…NoContent`) already PASS so Piece B alone flips nothing extra. **METHOD that produced round 291's +9: run 2-4 read-only multi-agent hunts (§4) per session over un-examined NONE/SWAP slices, asking "every error AST-derivable via a corpus-unique additive walker?"; EMPIRICALLY verify each SCOPEABLE in isolation before commit (agents over-rate ~10%, esp. gate-broadenings). The vein is NOT exhausted — round 290 wrongly declared it so, round 291 found 9.** The remaining ~30 NONE/SWAP that hunts RE-CONFIRMED ENGINE (do NOT re-hunt): intersectionsOfLargeUnions, constraintWithIndexedAccess, narrowingMutualSubtypes, knockout, circularlyConstrained…, duplicatePackage, underscoreTest1, mappedTypeWithAsClause…, extractInferenceImprovement, recursiveTupleTypeInference, inferenceFromIncompleteSource, normalizedIntersectionTooComplex, deeplyNestedMappedTypes, promisePermutations(2), awaitedType, reactReduxLikeDeferredInference…. The historical content below is superseded — consult only for deeper detail.
