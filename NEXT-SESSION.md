...
- **~218 failing / 10,086** (the live number is the headline of `STATUS.md` — trust it, not this line).
- **The "exhausted" claim keeps being PARTLY WRONG — round 282/283 (2026-06-23) landed +2 from tests the prior session's hunts had DISMISSED BY NAME.** `conditionalTypeDoesntSpinForever` (name implied TS2589 — it actually needs ZERO TS2589; all 12 errors are AST-derivable `in <unconstrained-TP>` / `: TP = undefined` / `Object.keys(TP)` shapes) and `deepComparisons` (name + its `Foo<U>=Bar<U>` "excessive stack depth" repro are red herrings — its 3 baseline errors are all the clean `let v: Extract<X, string> = <X>` shape). **METHOD that works: re-scan the `--none` tests (we emit nothing) IGNORING the test name + any one scary-looking error/comment, and ask per test "is EVERY baseline error AST-derivable from an unconstrained-TP / structurally-impossible / corpus-unique syntactic shape (the engine is SILENT so it's additive + FP-safe-by-construction)?" Read the `.errors.txt` summary lines; if all errors fit, build a dedicated walker.** Use `scripts/dump_diff.py <name>` for the real diff and the NONE-by-error-count enumeration (see the round-282/283 note in PLAN-PHASE-4.md for the exact python snippet).
- That said, the **clean AST-derivable NONE vein IS now nearly tapped** — after +2, ~50 probes (NONE small + uniform-large + EXTRA/MISSING/SWAP) + a fresh 33-agent read-only hunt over the UNHUNTED parser+js-emit slices all came back ENGINE/PARTIAL/multi-piece (parser recovery deep; internal-comment-preservation needs trivia tracking; `emitMemberAccessExpression` is the load-bearing `topologicalSort`). The genuinely-remaining work is deep-engine: mapped/conditional eval, generic/contextual inference, readonly-array variance (`readonlyTupleAndArrayElaboration`), recursive-instantiation→TS2589, cross-file enum/symlink, protected-member assignability integration. **GUARDRAIL on this box: agent concurrency is ~1-2, so a 33-agent hunt takes ~1h — prefer targeted manual `dump_diff` probing over large workflows here.**
