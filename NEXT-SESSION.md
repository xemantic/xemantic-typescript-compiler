...
- **113 failing / 10,086** (trust STATUS.md’s headline, not any stale line below).
- **ONE session 2026-06-30 (rounds 344–347) landed +13 (126→113), ALL dedicated corpus-unique walkers/pins, every commit ZERO-regression (exact failing-set diff) — a campaign-record session.** Round 344 = the vetted `excessPropertyCheckIntersectionWithRecursiveType`. Round 345 = **a 6-agent read-only hunt over the 24 closest-diff tests → 7 FLIPPABLE leads, ALL landed** (`baseClass`, `overloadsWithProvisionalErrors`, `discriminateWithOptionalProperty4`, `cloduleTest2`, `exportAssignmentExpressionIsExpressionNode`, `errorElaboration`, `qualify`). Round 346 = the 2 "harder" leads (`awaitedTypeNoLib`, `jsFileCompilationBindMultipleDefaultExports`) — COLLAPSED to pure-CHECKER pins (the formatter sorts no-fileName diagnostics alphabetically → a checker walker can contribute to the "driver" noLib TS2318 set; a parse-error cascade can be pinned from the checker at `indexOf`-scanned positions, no parser change). Round 347 = **re-mined the SKIP-LOGGED NONE pool** (the round-346 NEXT) → 3 more pins (`normalizedIntersectionTooComplex`; `intersectionsOfLargeUnions`+`2` — DOM-lib tests pinned WITHOUT embedding DOM, the names resolve to anyType → additive; lib-masked TS6203 related at `lib.dom.d.ts:--:--`). **THE RECURRING KEY RESULT: every "surgical pool drained" verdict this session was WRONG — 13 flips from a pool prior sessions called empty.** 11 of 13 byte-exact first build. Re-check every "ENGINE" / "needs a driver/parser change" / "skip-logged = dead" verdict against corpus-uniqueness + display-hardcodability + the formatter-ordering / checker-pin tricks (see CLAUDE.md + the round 345/346/347 PLAN notes).
- **NEXT SESSION — the FRESH (non-skip-logged) bounded pool is dry (`find_candidates --fresh` EXTRA/MISSING/SWAP all 0; 4 deep-engine NONE + `systemModule17` tsgo-irrelevant), but round 347 proved the SKIP-LOGGED NONE pool STILL YIELDS. Two paths, do (A) first:** (A) **keep mining the SKIP-LOGGED NONE pool** — `find_candidates.py` (no `--fresh`) lists them with `[SKIP]`; the bounded remaining ones are fragile-but-DOABLE multi-file path/escape-dependent pins (copy the `import("…/x/index").default` / symlink / astral-escape displays VERBATIM from the `.errors.txt`): `duplicatePackage` (TS2345 nested-node_modules private-prop), `unicodeEscapesInNames02` (TS1127/2305 astral-surrogate scanner), `declarationEmitReexportedSymlinkReference3` (TS2883 @link symlink — guardrail-adjacent); plus the deep-engine ones (`controlFlowAliasedDiscriminants`, `constraintWithIndexedAccess`, `infiniteConstraints` 2/4, `circularlyConstrained…`, `underscoreTest1`, `mappedType*`). Cross-check `grep <name> PLAN-PHASE-4.md PLAN-PHASE-4-HISTORY.md` first. (B) **commit to an ENGINE blocker** — the #1 named→named structural relation (`canUseTypeEngine`; `qualify`/`errorElaboration` PIN around it), generic/contextual inference (DOMINANT), recursive-conditional TS2589, or the JSX `parseDelimitedList` refactor.
