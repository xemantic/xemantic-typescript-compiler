...
- **~210 failing / 10,086** (the live number is the headline of `STATUS.md` — trust it, not this line).
- **Round 290 (2026-06-23) landed +1 — `distributiveConditionalTypeConstraints` (B564), the FIRST bounded engine-blocker slice: a dedicated walker that resolves a deferred conditional-alias param (`x: Cond<T>`, `type Cond<P> = P extends Ext ? <litA> : <litB>`) via T's CONSTRAINT (assignable→true / disjoint→false / overlap→boolean, per-union-member overlap load-bearing) and checks it against a literal-target var-decl → TS2322. ADDITIVE (getTypeFromConditionalType bails to anyType for a bare-TP checkType). This PROVES bounded slices of conditional-type eval ARE landable — look for sibling shapes. After this, the cheap pool is EXHAUSTIVELY dry: this session directly triaged ~30 candidates (7 fresh NONE, 5 SWAP, parser cascades, weak-type/mapped/flow/intersection smalls) + a 4-agent hunt (17 candidates → 1 SCOPEABLE [B564], 2 over-rated PARTIAL, 14 ENGINE). `enumAssignmentCompat6` and `controlFlowAliasedDiscriminants` were agent over-rates (NOT bounded). NEXT: continue read-only hunts over un-examined slices for the occasional additive walker, else attack a Blocker substep (the conditional-eval vein is the most-proven; mapped-eval `getTypeFromMappedType` ~118669 has NO per-key K-binding — a real gap but won't single-flip).**
- **Round 289 (2026-06-23) landed +1 more — `fuzzy` (B563), a namespace-local `implements` (TS2420) + `this`-valued returns (TS2322/TS2352) walker — bringing the session to +5 (216→211).** It REGRESSED 9 top-level `implements` via double-emit on the first run (my crude piece-1 TS2420 fired for top-level interfaces the shared `checkImplementsClauses` already owns); fixed by gating piece-1 to `!globals.containsKey(ifaceName)` + class/interface-no-`extends`. **After this, the fresh additive (`--none --fresh`) pool is down to 7 tests, ALL deep-engine** (`reactReduxLikeDeferredInferenceAllowsAssignment`, `deeplyNestedMappedTypes`, `complicatedIndexedAccessKeyofReliesOnKeyofNeverUpperBound`, `promisePermutations`/`2`, `awaitedType`, `controlFlowArrayErrors`). The dedicated-walker vein is now genuinely thin — a hunt MIGHT still find a corpus-unique additive shape, but expect ENGINE.
- **TOOLING WARNING (cost real wall-clock this session): a concurrent `/home/abe` linter runs `./gradlew` (a `timeout 320` build) and competes for memory on this 7.7GB box — it caused 2 OOMs + 1 daemon-failure. Before each build: `./gradlew --stop; pkill -9 -f KotlinCompileDaemon` (kill YOUR orphan ~2.5GB kotlin daemon, NOT the abe procs) and wait for `ps aux | grep [h]ome/abe.*java` to drop to ≤1 + `free -m` >3500MB. Build with `run_in_background` and react to the notification.**
