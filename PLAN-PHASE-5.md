...
- [x] **(CHK.133) CLOSED 2026-09-12 ((P18.79) note: (b) the relation's `this` leg — `Relater.signatureThisTypesRelated`,
  ...
  (b)** the relation's `this` leg (tsc `compareSignaturesRelated`: a source `this` other than `void` must relate to the
  target's, bivariant unless `strictFunctionTypes` and a non-method; chain line *The 'this' types of each signature
  are incompatible.*), and the **`.call/.apply/.bind` consumer** (lib `CallableFunction` under `strictBindCallApply`;
  today `f.call` types `any`). Residues: an optional-chain receiver (`o?.f` types `any`), a `this` type mentioning a
  type parameter (no inference at the check), overload sets not `this`-checked, one-level elaboration. ORIGINAL:
  `Signature.thisParameter` — THE MODEL CHANGE (CHK.97) D6 IS BLOCKED ON (promoted 2026-09-12, (P18.77)).** `Signature` (`Type.kt:318`) has no `thisParameter`, so a `this:` pseudo-parameter is dropped by
...
- [x] **(CHK.97) CLOSED 2026-09-12 ((P18.78) note: D6 landed through (CHK.133)(a) — `unionTypeCallSignatures5/6` byte-identical to pristine; D1 rejected on a measurement; D2/D2b/D3/D4/D5 in (P18.71)-(P18.77)). D5 CLOSED 2026-09-12 ((P18.77) note) — inference through a union-combined signature: PASS 2's
...
- **(CHK.35) — DECOMPOSED 2026-09-22 ((P18.168)); (a) LANDED, `marked` 8 -> 4.** The 5 rows are
  TWO clusters with different causes, and the item's "one contextual-signature path" framing is
...
- [x] **(CHK.150) CLOSED 2026-09-23 ((P18.182) note) — rung 3 (X1) was the byte-parity guard discarding
  an overload's clear winner, and the `Partial<Observer<W>>` leak was `instantiateMethodParamType` pushing
  a mapped bag through plain `instantiateType`; both fixed, **`rxjs` 17 -> 10**, the predicted seven rows
  exactly. Residue e17 (tsgo fixes a callback parameter from the FIRST overload and reports TS2769) left
  unpinned; rxjs's remaining ten rows are separate families listed in the note. RUNG 1 (X2) LANDED 2026-09-23 ((P18.180) note) — a generic reference's members are
...
- [ ] **(CHK.167) ROUND 1 LANDED 2026-09-24 ((P18.192) note; 100 missing rows now report, +0 everywhere).
  ...
  target matches no rule and returns false ("often need narrowing we don't implement", 1c7db1bd3, 2026-04-06,
  pre-dating suppression-only narrowing; no measured FP family recorded) — refusing declaration, assignment,
  return and property-assignment; the ARGUMENT reader has a second firewall (`caasNonSimpleParamChecks`, no
  `allow*` claims a union argument); object-literal members already report; nullish unions at assignment already
  report (B86.7d). Matrix 35 source x target x 11 readers + 15 narrowing families: with a per-member "lift"
  opening ~98% of rows land with tsgo's exact line/col/code (differences: chain depth; property assignment prints
  no chain); array->tuple stays refused by design; weak type `W` relates wrongly (separate). Narrowed-at-site
  cells stay clean EXCEPT four gaps that become FPs: (a) `let v: U = init` — the initializer does not narrow the
  declared type; (b) equality against an object constant `x === kk`; (c) element-access narrowing `a[0]`;
  (d) discriminant narrowing of anonymous-object unions at decl/assignment (PARITY.1(c)'s all-primitive clause).
  **Real code** (8 profiles, rxjs, marked, cronstrue): naive all-readers+argument +28-35 per profile, 36 distinct
  rows, **0 tsgo-matching** (24 nullish — `x.p ??= v; return x.p`, lets assigned on every branch, `return
  result!`; 12 non-nullish — conditional/`||`/call sources and reference-source narrowing gaps: typeof on
  branded-string intersections `checker.ts:3274`, a never-returning `Debug.fail` ending a branch
  `fourslashImpl.ts:1248`, an aliased condition `const isO = guard(x)`, a generic guard inside a ternary);
  non-nullish-only 6-11 per profile; **the round-1 shape 0 everywhere**; corpus under JDI 2 / 1 / 0 mismatches
  for the three arms. **ROUND 1**: open `canUseTypeEngine` for a union source vs an object-family target when
  EVERY member is individually admitted (keeps array->tuple refused) and NO member is null/undefined; readers
  declaration, assignment, property assignment ONLY; source an identifier or property access. In the same
  commit extend the anonymous-object narrowing clause to object-carrying unions (the four `nt_aoo` FPs) and
  fix or refuse (a) and (b); pin all three. Predicted ~20 cells x 5 readers fixed, +0 everywhere. **ROUND 2**:
  return + argument readers (argument arm needs rest/arity refusal — `push(x)` on a union-alias element was an
  FP), after the four reference-source narrowing gaps and the generic-assertion one (`assert(pred(foo))`,
  `strictSubtypeAndNarrowing:204/209`); keep call/conditional/`||` sources refused. **ROUND 3**: nullish unions
  ((CHK.63) territory, 24 rows; needs `??=` narrowing, every-branch let joins, `!` in the return reader).
  Side findings: (CHK.168) filed; array-literal elements against `T[]` miss even for primitive unions; weak-type
  check missing for union sources (`oo_W`/`po_W`). EARLIER: A UNION SOURCE AGAINST AN OBJECT OR ARRAY TARGET IS SILENT AT DECLARATION AND ARGUMENT
...
- [ ] **(CHK.168) ROUND 1 LANDED 2026-09-24 ((P18.194) note; 28 cells, 0 FPs, +0 everywhere). OPEN: round 2
  ...
  answering `any`, so it exits at `bodyType === anyType`; a literal reports only because its type is
  scope-free (CLAUDE.md's "a per-node hook on the spine sees none of the checking ambient"). The scoped walker
  `walkFunctionBodiesInExpr`'s expression-arrow branch (~102033) installs the parameters but never
  return-checks. tsgo runs the SAME `checkReturnExpression` for an expression body
  (`checkFunctionExpressionOrObjectLiteralMethodDeferred`, checker.go ~10182; async-unwrapped, error node the
  paren-stripped expression, conditional branches separately). 131-cell matrix with two JDI-forced arms: mode 1
  (re-run the concise check in the walk) fixes 25 but adds 3 FPs (no async unwrap, no un-inferred-TP guard);
  **mode 2 (hand the body to the block-body `return` path inside the walk) fixes 25, 28 of 28 clean controls
  stay clean (every (CHK.167) narrowing risk included), +0 rows on all 8 profiles, rxjs, marked, cronstrue**;
  corpus 1 mismatch — a DUPLICATE (`conditionalReturnExpression:19` emitted 3x: the spine once, the walk twice,
  because the walk runs twice per top-level statement from `ctaM3StmtAnchorCore` ~3459 and
  `checkConditionalReturnBranches` adds rows directly). **ROUND 1**: in the expression-arrow branch, when the
  arrow has a return annotation, route the body through the block-body return machinery with
  `checkFunctionBody`'s flags (`inAsyncFunctionBody` from the arrow, generator off), the contextual parameter
  pull `applyPulledContextualParamTypes(arrow, parameters)` (what makes `take((x): number => x)` report), the
  anchor at the paren-stripped body passed into `checkReturnAssignabilityCore` (which hardcodes `stmt.pos, 6`
  — the `return` keyword width — at ~10 sites), single emission (dedupe `checkConditionalReturnBranches` or gate
  it on the record-only pass), and RETIRE the spine call. Pins: `k_cond`, the mode-1 traps `k_async_ok` /
  `ok_promise_nonasync` / `ok_arr_map`, `paren`, `weak`, the clean controls. **ROUND 2**: class-property,
  `static` and default-export arrow initializers are walked by NOTHING (block bodies silent there too).
  ROUND 3: the member-drill display (3 cells, shared with the block path). Separate: `<T>(x: string): T => x`
  missed by the block path too (foreign-TP gate). EARLIER: AN ANNOTATED ARROW'S *EXPRESSION* BODY IS NEVER RETURN-CHECKED FOR A NON-LITERAL
  EXPRESSION — `export const f = (x: string): number => x` is SILENT where tsgo reports TS2322 (found by the
  (CHK.167) census, re-probed 2026-09-24 by the orchestrator, `build/scratch-orch-p190`).** A block body
  (`=> { return x; }`), a function declaration, a function expression and a LITERAL expression body
  (`(): number => "s"`) all report; a class-property arrow with an expression body is silent too. Plausibly the
  return reader handles `ReturnStatement` and the arrow-expression-body arm only covers literals. A visible
  false negative in the most common modern function form — census the reach (tsc's sources and rxjs are full
...
- [ ] **(CHK.152) STEP 3 LANDED 2026-09-24 ((P18.198) note: narrowed-receiver second chance + union/nullable parameters, +0 everywhere). OPEN: step 2 (rest element), step 4 (union arguments — (CHK.161)(c)'s U4 arm is the measured shape), Round B. STEP 1 LANDED 2026-09-23 ((P18.183) note) — named object vs named object is related at
  ...
  foreign-type-parameter, rest, arity) for every `CAAS_CONTINUE` exit — so a prediction is exactly the set
  of sites an opening would reach with a failing relation (diagnostic counts unchanged under the debugger:
  the control). **STEP 1 = `allowNamedObjectVsNamedObject`**: argument `Type.Interface` / non-array
  `Type.Reference` / `Type.Intersection` (never a literal, arrow or function expression); parameter
  `Type.Interface` / non-array `Type.Reference` / aliased `Type.Object` / `Type.Intersection` (**NEVER a
  union**); the three (CHK.103) guards (rest position, `arityOk`, no foreign type parameter either side —
  precedent commit `4e0079c2`, `allowArrayLikeVsArrayLike`); decision `canUseTypeEngine(argType,
  paramType)`, whose Object↔Object branch is what declaration position already uses. **AND THE CHAIN, which
  the item did not name**: `caasTailGatesAndRelation`'s `getPropertyElaborationChain` call excludes
  `Type.Interface`/`Type.Reference`, so an opening alone prints the head with NO chain — relax it or reuse
  `cvdaElaborateMismatch`, pinned against the declaration chain (24 of 28 already equal tsgo; expect the
  known `strictFunctionTypes` function-property form divergence). **Predicted**: 27 of 32 argument misses in
  a 47-cell matrix close (`matrix-cells.tsv`); profiles 0 (tsc-project: 7,494 narrow pairs, 35 relation
  failures, all removed by the rest/foreign-TP guards; harness 14,311 / 50 / 0); rxjs 191 pairs 0, marked
  0, cronstrue 0; a 154-fixture corpus sample 0 added / 0 FP / 0 duplicates (thin — 31 admitted pairs — so
  the corpus screen still gates duplicates against pin walkers). **Traps measured**: `ctl-arity-wrong`
  reaches the relation (without `arityOk` a TS2345 lands beside tsgo's lone TS2554); `pos-rest` compares
  `Q` against `S[]`. **A WIDER OPENING IS A FALSE-POSITIVE SOURCE ON REAL CODE**: union arguments with an
  un-narrowed `undefined`, named argument vs union parameter, array literals vs union, `ReadonlyArray`
  targets — 7 sites on tsc-project, 38 on harness; function-typed arguments add 23 FPs in the corpus sample
  (`() => void` vs `Function`). **Later steps, each its own census**: (2) rest-element object path; (3)
  union / nullable parameters after a narrowing second chance like `argArrayLikeNarrowsToRelated`; (4)
  union arguments. **Corrections**: rxjs's `WebSocketSubject.ts:304` is NOT this family (it misses at
  declaration too — an `ArrayBufferView<ArrayBufferLike>` relation gap); the corpus population is tiny (23
  named→named TS2345 rows in 19 baselines, 11 active and matched), so pins + the cell matrix are the gate.
  ORIGINAL: **A NAMED-OBJECT ARGUMENT IS NEVER RELATED TO A NAMED-OBJECT PARAMETER — `caasNonSimpleParamChecks`'
...
- [x] **(CHK.153) LANDED 2026-09-23 ((P18.184) note; rxjs 10 -> 7). `this` IN A CALLBACK: RESOLVE THE CALLEE'S RECEIVER FROM THE IDENTIFIER'S PARENT CHAIN IN
  ...
  BODY-LOCAL (`scheduler.schedule(function () { this… })`) cannot be resolved and the `this:`-carrying
  parameter is never seen; a file-level receiver works — which is also why (P18.177)'s typing fix moved
  none of these rows. Reducers `build/scratch-p18183-census/cells/this1` (param), `this5` (body-local); controls `this2`, `this4`. **This
...
- [x] **(CHK.154) CLOSED 2026-09-23 — (a) ((P18.185) note; rxjs 7 -> 6) and (b) ((P18.186) note: own construct signatures only, plus the two relation defects it exposed; residues in (CHK.163)). A TRAILING PARAMETER ACCEPTING `void` IS OPTIONAL IN SIGNATURE RELATION — rxjs
  ...
  `(value: void | PromiseLike<void>) => void` must relate to `() => void`. Reducer `void9`/`void9c`,
  `void2`. Size S. (b) it surfaces only because a DERIVED class's `constructSignatures` carry the BASE
  constructor first and only `constructSignaturesForNewCtx` filters to the class's own — so
  `class Sub extends Base { constructor(o?: number) }; new Sub("x")` is ACCEPTED here (tsgo TS2345;
...
- [x] **(CHK.155) LANDED 2026-09-23 ((P18.187) note; rxjs 6 -> 5). A CAPTURED READ IN AN EXPRESSION-BODIED ARROW IS CHECKED AGAINST THE OUTER UNINITIALIZED
  ...
  (`isOuterVariable && !isNeverInitialized`, checker.go ~11119). Block-bodied arrows, function
...
- [x] **(CHK.157) LANDED 2026-09-23 ((P18.189) note; rxjs 4 -> 3; emitter was the SPINE, not the named legacy arm).
  ...
  early-exit install ~4207 already does) and allow a negated guard call. (CHK.64)(ii) covered only the
...
- [ ] **(CHK.159) STEPS 1-2 LANDED ((P18.191), (P18.193): rxjs 2 -> 0 ours-only). OPEN: the later steps below
  ...
  **Later**: the `ctxReturnTypeParamMapper` fallback (groupBy, `operate`/`createOperatorSubscriber`);
  ...
  (fully) from the contextual return; the raw callee type survives and is hidden by
  `typeContainsForeignTypeParam` ONLY when its name differs from the caller's — most rxjs operators pass by
  that coincidence. Controls: renaming the caller's parameter makes both SILENT (blind, not correct);
  `f<T>(x: T | T[]): T[]` called with a number is silent here and reported by tsgo. Reducers `build/scratch-p18183-census/cells/race1`,
...
- [ ] **(CHK.160) CENSUSED 2026-09-23 (read-only, frozen (P18.184) classes, `build/scratch-p18185-census/`) — THE
  ...
  `grp7` (a generic ALIAS of a function type as a call's return is never instantiated, even with explicit
  type arguments) are one mechanism. A function-aware path exists (round 465's `instantiateTypeFnAware`);
  the question is why plain `instantiateType` skips and what making it function-aware would move — a
  whole-checker change, so census first (the (CHK.151) census measured that putting type parameters in
...
- [ ] **(CHK.161)(c) CENSUSED 2026-09-24 — THE LAST rxjs ROW NEEDS *FIVE* FIXES, NOT TWO, AND TWO OF THE NEW ONES
  ...
  **B6** — `ccetEnterClassDeclaration` (~1835) looks a class up only in `globals`, and a MODULE-FILE class is never
  ...
  where tsgo reports (`sub4` line 6); (c) the MISSED tsgo row `WebSocketSubject:304`: a nested bare
  generic reference whose parameters are all defaulted (`ArrayBufferView`) is left un-instantiated and
...
- [ ] **(LIB.5) 2026-09-24 ((P18.193)): `rxjs` reads 0 OURS-ONLY ROWS (29 at (P18.173)); ONE tsgo row is still
  ...
  **G3 LANDED 2026-09-23 ((P18.175) note) — `rxjs` 21 -> 17.** The family proved WIDER than the
  sizing: besides an arrow and a function expression, a `FunctionType`, a `TypeLiteral` method
  signature and a `ConstructorType` are the same mechanism with the same measured tsgo silence —
  one helper at SIX boundaries, and the type half is separately pinned by its own ablation arm.
  The corpus turned out to be a LIVE gate, proved by the arm that subtracts the WRONG set (the
  screen goes 0 -> 1 and names `genericClassWithStaticsUsingTypeArguments`, which loses exactly
  its two ARROW rows). **Three adjacent MISSING-row defects were found and NOT fixed**, each
  needing its own round because each ADDS rows: a type-parameter CONSTRAINT is never walked, a
  nested `FunctionDeclaration` statement is never walked, and an arrow's or function expression's
  BLOCK body is never walked. **`rxjs` is now 17 vs tsgo's 1, with G2 (7 rows, (CHK.35c)) and G4
  (3 rows, re-scoped under (CHK.141)) still owned and 7 unrelated singletons left.**
  ORIGINAL SIZING: **G3 is bounded**: TS2302 is answered by
...
- [ ] **(LEGACY.0) (0a) + (0b) STEPS 1-44 LANDED 2026-09-21 ((P18.85)-(P18.155) notes) — pending **19**,
  ...
  **6** active baselines with >=2 consecutive `Types of parameters` lines and **0** with >=3;
  ...
  where the identical `{ m: () => void }` reported. **The axis is NOT namespace-qualification** (the round opened on
  that hypothesis and it is false — `namespace A { const foo: () => void }` + `A.foo.bar` has always reported); it is
  whether the type carries a declaration SYMBOL. **tsgo has no checker-side expando exemption at all** — its BINDER
  declares expando properties onto the host symbol and TS2339 falls out of ordinary lookup, the host predicate keying
  on the head's `valueDeclaration` KIND at EVERY hop of a dotted name, which is why a namespace-qualified head
  declares at none and why six probe shapes changed answer between 6.0.3 and 7.0.2. **We SUPPRESS where tsgo
  DECLARES** (measured: `function z(){} z.px = 1; const s: string = z.px` is silent here, TS2322 in tsgo, and we
  never emit TS2565), and that is what bounded the round: only the PROPERTY-ACCESS-receiver route (A) landed.
  **ROUTE (B), the IDENTIFIER receiver (`cmamCheckResolvedObjectType`'s empty-properties branch, whose only emission
  is the B63.33 `'{}'` case and REQUIRES no call signatures), MUST NOT BE OPENED UNTIL EXPANDO MEMBERS ARE
  MODELLED** — it is the only reason `const f = () => {}; f.bar = 1` is correctly silent, and it is (CHK.124)'s
  "widening B431's candidate scan", the named successor. The guard was built UNGUARDED first and fires on five
  attributable cells; the pre-gate refuses a class static side and a function⊕namespace merge, both LOST ROWS and
  not false positives, pinned `residue -`.
  PREVIOUS HEAD: (0a) + (0b) STEPS 1-25 LANDED 2026-09-17 ((P18.85)-(P18.131) notes) — pending **40 → 37**,
  ...
  not a resolvable same-file `ClassDeclaration`); J2 added the class STATIC side — **the function half already
  existed as B431**; and **B428 IS RETIRED**, measured with the PassLab (without it `jsCheckObjectDefineThisNoCrash`
  double-emits). **J4's axis in the (P18.130) table was WRONG**: it is not JSDoc `@template` but a CONSTRUCTOR-context
  display defect in plain TypeScript — a constructor body's member table has not resolved, so the read lands on
  `cmamCheckResolvedObjectType`'s `ctorClassSym` fallback rather than `cmamEmitMissingProperty`, and the fix is
  five lines. Two pre-existing `.ts` divergences were found and refused with their measurement: TS2551-plus-suggestion
  for a missing STATIC, and `extends Object` ((CHK.51)'s heritage firewall).
  **THE RESIDUE (39)**: display/chain-content ~19; F6-code ~14 (~14 MECHANISMS — size by mechanism, never by
  ...
  **THE JS ARC, AS (P18.131) LEAVES IT** — J1, J2 and J4 are DONE, and what is left is:
  - **J1b the TYPE-decided receiver** (NEW, and it is what all four J1/J2 residues share): an instance-typed
    VARIABLE receiver, a JavaScript OBJECT LITERAL, a base that is not a resolvable same-file `ClassDeclaration`,
    and `X.prototype.p = 1`. Each needs the receiver's class decided by its TYPE rather than syntactically.
  - **J3 TS7053 for element access** — the ONLY row left on `classFieldSuperNotAccessibleJs`, and **not a
    JavaScript question**: tsgo anchors a two-line chain at the whole access where our element funnel gives TS2339
    at the index, and the same divergence reproduces in a `.ts` file.
  - **J5 the ccet pair**, three independent sub-items: the TS2349 double-emission's ownership; a JS class with no
    ctor extending a `.d.ts` class has no construct signature (3 false TS2351); JSDoc `@overload` sets are not
    modelled (1 false TS2345). The TS7009-from-the-callee-type residue is NOT gated by this — it is silent in
    `.ts` too.
  - **J6 a `declare class` receiver** and the TS2551 static-suggestion gap — both pre-existing, both reproduce in a
    `.ts` file, so neither is in this family at all.
  - **J7 an unchecked JS file should report nothing** — `allowJs` without `checkJs` still leaks a TS7006, and
    (P18.123) recorded a TS2855 twin; several walkers key on the EXTENSION rather than on `checkJs` ((P18.92)).
  **THEN**: the TS2749-on-a-value-only-import half of `jsExportMemberMergedWithModuleAugmentation`; then the
  TS2403-vs-TS2717 split ((P18.120)). **PICK AND SIZE WITH `bash scripts/corpus-screen.sh`**, and count the ACTIVE
  subtests carrying each code first (146 of 2,842 carry a `.js` file, of which **16** carry an expando assignment —
  that 16 is the nameable at-risk set for anything in this family, and (P18.131) verified every one individually).
  PREVIOUS HEAD: (0a) + (0b) STEPS 1-22 LANDED 2026-09-17 ((P18.85)-(P18.123) notes) — pending 40 (UNCHANGED, 2
  rewritten), skipped 65, suite 19,741/0. **(P18.123) landed the JS-expando DECLARATION rule and closed NO row —
  both are PARTIAL**: only an ASSIGNMENT declares (`=` and an access LHS, so `this.c += 1` and `(this.f) = 3` do
  not), the REPARSER drops a `@type` tag above a non-declaration statement, and the TS6205 aggregation is over the
  DECLARATION's whole type-parameter list rather than per tag. Both rows are strict SUBSETS of tsgo's.
  **(P18.121)'s recorded "the aggregation predicate is already correct" was FALSE and this queue had copied it
  forward into a brief** — re-derive before quoting.
  ...
  **NEXT CLUSTER BY MECHANISM**: `nodeNextPackageSelfName*` ×2 (nodenext self-name resolution),
  ...
  excessive-complexity head suppression; TS2615 no longer paired with TS2589; the `bigintWithLib` pinDiag chains
  re-transcribed). **THREE OF SIX "MECHANISMS" WERE tsc-6 TRANSCRIPTIONS** (a `pinDiag` wipe-and-pin walker, a
  one-fixture walker, a paired emission) — before designing a fix for a pending row, run the PassLab `disable
  <pass>` preview on the emitter `--passTiming` names; a row that goes to 0 mismatches was never an engine
  question. **THE DECOMPOSED RESIDUE (58)**: display/chain-content ~19; F1 silent 4; F2-residue 6; ORDER-model 5;
  TS2683-residue 3; JS emit 3; the two (LEGACY.1) questions (`downlevelIteration` TS5102 ×2, `pathsValidation5`
  order); the three M2 partials (TypeScript 7 types `exports.p` after `module.exports = X` as a property access
  on X's type — one family); `controlFlowInstanceof`'s three `instanceof` residues (sized in the (P18.101) brief:
  a `Function`-typed RHS narrows by `{}` and drops `null`; a non-overlapping class gives an INTERSECTION the join
  reduces; a checkJs `@constructor` instance type); `nodeNextPackageSelfName*` ×2, `tslib*MissingHelper` ×2 +
  `esModuleInteropTslibHelpers` (one tslib-helper-lookup mechanism, 3 rows), `isolatedDeclarations*` ×2; the rest
  singletons — SIZE BY MECHANISM, expect ≤6 per round. **PICK AND SIZE WITH `bash scripts/corpus-screen.sh`** and
  grep the 21 tsc-6 MIRRORED BASELINE FILES for every code a round will move — AND grep the hand-written test
  SOURCES for the codes in their ASSERTIONS ((P18.101) lost a suite run to five countdown pins whose NAMES named
  no code). **A KNOWN FOLLOW-ON**: three hand-written pins differ from tsgo in CODE because our relation CHAIN line
  names the type parameter / undistributed intersection where tsgo names its constraint / one distributed
  constituent — that chain SOURCE DISPLAY is its own family. The 21 TS-1 rows stay LEDGERED.
  **BLOCKED-PENDING-USER, still open**: the fourth "harness artifact ⇒ fall back to tsc" arm ((P18.86)).
  PREVIOUS HEAD: (0a) + (0b) STEPS 1-15 LANDED 2026-09-14 ((P18.85)-(P18.100) notes) — pending 66, skipped 90,
  ...
  **BLOCKED-PENDING-USER, still open**: the fourth "harness artifact ⇒ fall back to tsc" arm ((P18.86)).
  ...
  for every rendering change left: grep them BEFORE a family round. **THE DECOMPOSED RESIDUE (84)**: display/
  chain-content ~21 (6 type-DISPLAY + 1 chain-CONTENT reclassified rows, the `Object` hint in argument/return
  position, the tie-break by declaration order); F8 span 12 (TS5053 ×2, TS2880 ×2); F1 silent 8 (TS1003 ×3,
  TS2339 ×3); F2-residue 6 (four mechanisms, named at (P18.93)); ORDER-model 5 (named at (P18.94)); TS2683-residue
  3; JS emit 3; the `downlevelIteration` TS5102 pair (2 — an owner decision under (LEGACY.1), would redden nothing);
  the rest singletons — SIZE BY MECHANISM, never by F-letter, expect ≤6 per round. **PICK AND SIZE WITH
  `bash scripts/corpus-screen.sh`** (8,746 subtests over errors+emit in ~70 s). **A KNOWN FOLLOW-ON**: three
  hand-written pins differ from tsgo in CODE because our relation CHAIN line names the type parameter /
  undistributed intersection where tsgo names its constraint / one distributed constituent — that chain SOURCE
  DISPLAY is its own family and closing it also closes those three. The 21 TS-1 rows stay LEDGERED.
  **BLOCKED-PENDING-USER, still open**: the fourth "harness artifact ⇒ fall back to tsc" arm ((P18.86)).
  PREVIOUS HEAD: (0a) + (0b) STEPS 1-12 LANDED 2026-09-14 ((P18.85)-(P18.97) notes) — pending 92, skipped 117,
  ...
  **BLOCKED-PENDING-USER, still open**: the fourth "harness artifact ⇒ fall back to tsc" arm ((P18.86)).
  ...
  count**: F6z 33 (a `checkJs` slice may be cheapest), JS emit 33, F6a 28 (**REFUSED (P18.88) on a measured
  blocker — the missing-property message has ~30 independent emission sites and no relation-error funnel, reach 73
  of 2,955 active baselines; the FUNNEL is its unblocker and should be promoted before re-attempting**), F3
  last-overload 25, F1 11, F8 unrelated-anchor residue ~11 (TS5053 ×2, TS1356, TS2447, TS2880 ×2, TS1092, TS1517 —
  a DIFFERENT emitter from the anchors (P18.88) landed), F6d 10, F2 9, F0 7. The 21 TS-1 rows stay LEDGERED.
  **BLOCKED-PENDING-USER before (0b-3) — still open**: the fourth "harness artifact ⇒ fall back to tsc" arm
  ((P18.86)). PREVIOUS HEAD: (0a) + (0b) STEPS 1-2 LANDED 2026-09-13 ((P18.85)/(P18.86)/(P18.87) notes) — pending 242,
...
- [x] **(LEGACY.1) CLOSED 2026-09-17 — EVERY SUB-STEP LANDED ((P18.102)-(P18.113), (g) at (P18.132)).
  ...
  - [x] (c) LANDED 2026-09-15 ((P18.103), `e81d07381`; tsgo is strict-ALWAYS — measured over six configs — so the four `== false` arms went and the MISSING deprecation row was added; the surviving `explicitNonStrict` `strict: false` disjunct is a tsc-6 residue for (LEGACY.0b)) — `alwaysStrict: false` — `Emitter.kt:229-231` (`"use strict"` suppression; tsgo's `usestrict.go` never
  ...
  - [x] (e) LANDED 2026-09-15 ((P18.106), `963a03381`: `CompilerOptions.effectiveModuleResolution` is tsgo's derivation and replaces five Checker copies; TS5070/TS2792 deleted (no tsgo emitter), TS5109 added; (f) inherits TS5071 — no tsgo emitter, now visible on `*×system` — and `resolveJsonModule`'s module-kind derivation; (g) inherits the emit-order ancestor walk at `TypeScriptCompiler.kt:~3357`, which two green emit baselines pin, and four `baseUrl == null` conjuncts that simplify to `true` first) — `moduleResolution: classic/node10` — `ModuleResolver.kt` has NO such arm; the family is `Checker.kt:49308`
  ...
  - [x] (h) LANDED 2026-09-15 ((P18.108), `0c956db1a`: already inert on the project path; six harness-only arms deleted incl. TS5074's conjunct and the last None/outFile coupling; `extractRelativeImports`' reference edges are tsgo's program order and STAY — the "only used when outFile is set" comment was stale; parse, `out` and the TS5101/5102 row kept; four pre-existing project-path emit divergences recorded in the note: TS5074 in a tsconfig context, TS5011 never reported, `rootDir` flattening, no `.d.ts`/`allowJs` emit) — `outFile` — `TypeScriptCompiler.kt:1615-1620` `transformOrder` + `:3075-3120` (reference-directive
...
- [x] **(CHK.121) CLOSED 2026-09-11 ((P18.68) note) — AND THE AXIS IS THE *INITIALIZER*,
  WHICH MAKES BOTH EARLIER SIZINGS OF THIS ITEM WRONG.** It was queued as "one cell of four"
  and re-sized to "six of seven shapes"; measured, `const v: ZzzCfg = zzzCfgV` in a body
  ALREADY reported (the flow-recovery helper serves an IDENTIFIER initializer) and so did
  `let`/`var`/nested-block/arrow, while `number[]`, a heritage interface, an intersection, a
  function type, a numeric index signature, an enum and an optional are silent at FILE LEVEL
  too, i.e. pre-existing firewall refusals that were never this defect. What was silent is an
  OBJECT-LITERAL, `new`, CALL, `as` or scalar-literal initializer.
  `cmamAnnotatedLocalReceiverType` (24 functional lines) is wired LAST at the `anyType` bail
  of `cmamGeneralReceiverType`, reusing `cmamAllMissingTrustedMember` and
  `cmamInGuardMayAddProperty` so it can only turn a SILENCE into a report. Receipt: the
  original fixture byte-identical to both references (4/4 where we had 1) and a 207-cell
  matrix (references agreeing on all 207) 111 → 107 missing, ours-only unchanged at 2.
  **Refused and PINNED AS REFUSALS**: a class instance, an array, `let`/`var` (const-only),
...
- [x] **(INV.0) STEP 10c — HERITAGE AND QUALIFIED NAMES: LANDED 2026-09-10 ((P18.64) note).**
  ...
  consult and **nothing moved**. What decides a base type's MEMBERS is
  `Checker.getTypeFromBaseTypeExpression` (a FOURTH resolver) and what decides the
  `implements` verdict is `checkImplementsClauses`' own probe (a FIFTH) — three resolvers
  where the item named one, each found by measuring an inert patch. 25-cell matrix against
  both references (which agree on all 25, file control 5/5 clean): **10 ours-only / 28
  missing → 4 / 20**; `interface extends` 2/6 → 0/2, `implements` 2/4 → 0/4 (the false
  whole-class TS2420 gone), qualified enum root 4/6 → 2/2. Five ablation arms, union 5 of 5.
  **Residue**: the `namespace` qualified root (its scope symbol has no `exports`, so the
  root is deliberately NOT adopted — no answer is worse than a wrong one there), a second
  TS2694 walker that still answers the OUTER enum for a shadowing qualified reference and
  renders `'"a".ZzzE'` where both references render `'ZzzE'`, and `extends`/`implements`'
...
- [x] **(CHK.73) (ii)+(iii) DONE 2026-09-21 ((P18.157) note) — AN EXTERNAL MODULE SYMBOL HAS A
  ...
  **(A) DONE 2026-09-21 ((P18.163) note) — TS2339 for a member absent from a module object,
  and (B) the display with it.** `moduleObjectMissingMemberDisplay` + `moduleObjectTypeDisplay`;
  all SIX of tsgo's probe rows at tsgo's codes and spans. **THE BLOCKER RECORDED BELOW WAS FALSE
  IN BOTH HALVES**: no star-following enumeration is needed (REFUSING suffices —
  `ambientSurfaceIsEnumerableForImport` already existed and was proved discriminating by
  ablation), and the clause named below sits in a function this population NEVER REACHES — the
  real gate was a blanket alias skip in `cmamCheckIdentSymbolValueGates`. Residues: the BASENAME
  vs tsgo's full path (decided, KDoc'd, needs a `-project` pin for all twelve sites); an ambient
  `export * from` whose target does not resolve; a `.d.ts` holding one `declare module` (TS2306
  in tsgo). ORIGINAL TEXT, kept because its hazard analysis is still right even though its
  unblocker was not: tsgo reports it for both the
...
- [x] **(CHK.33) CLOSED 2026-09-21 ((P18.164) note) — `marked` 18 -> 10, corpus screen 0 of
  8,725, grid 8x0, and the 10-case reference matrix now matches tsgo 7.0.2 on every code and
  message it produces (before: 10 rows, every one wrong). Round 446's declaration-arity
  recovery is now the SHARED `Checker.signatureDeclaredArity`; (CHK.97)'s union-callee reader
  had reintroduced the split verbatim, and the axis is the UNION receiver — the same method on
  a plain receiver was already correct, which is why a fixture without one is vacuous. TWO more
  FP classes of the same root cause fixed with it: a REST parameter whose own name is a binding
  pattern (read as ZERO-arity), and a member typed by a FUNCTION TYPE (an options-bag callback
  property). The `Expected N-M` inversion is pinned as a MESSAGE invariant — the `require` the
  item asked for is UNAVAILABLE, because every binding-pattern signature violates it by
  construction. **SUCCESSOR, SIZED AND MEASURED: an OVERLOAD SET with a destructured overload**
  picks the wrong overload and reports a confident TS2345 on legal code (`interface Ov { m({a}:
  O): string; m(x: number, y: number): string }`), through overload SELECTION
  (`resolveCallOverload` 129258-129269, `allArgumentsMatch` 163057) rather than a TS2554
  emitter; a census counts ~55 readers of the same split, so it is its own round. Two stated
  residues: the too-few rows anchor at the RECEIVER where tsgo anchors at the METHOD NAME, and
  a tuple-typed rest is not expanded to a fixed arity (`fixedTupleLengthOfRestParam` is the
  lever), so we are now silent where tsgo reports two rows — a false NEGATIVE traded for a
  false POSITIVE. ORIGINAL: A DESTRUCTURING PARAMETER BREAKS ARITY, AND THE MESSAGE PROVES IT: `Expected
...
- [x] **(CHK.140) CLOSED 2026-09-22 ((P18.167) note) — A CLASS'S OWN TYPE PARAMETERS ARE IN SCOPE
  INSIDE ITS MEMBERS' BODIES.** `ctaFnBodyFrame` fed `classTps` to `fnTpDecls` (the AST map, which
  answers TS2302) and never to `fnTpScope` (the map that TYPES a name), so a class type parameter
  failed to SHADOW a file-level type of the same name — a WRONG type, degrading to `any` only when
  no outer name existed. **(P18.166)'s title for this item named a downstream symptom**: it is not
  the member table, not `resolveGenericPropertyType` and not round 761's cached member type; the
  settling fixture has no member access at all. Static members are deliberately NOT excluded — the
  mirror of B74.5 was built, read 0 RED under ablation, measured LOSSY (tsgo reports TS2302 AND
  types the reference as `P`) and REMOVED. Corpus screen 0 of 8,725 (the real gate: 281 case files
  carry the shape); grid 8x0 but a CONTROL on six arms (84 lines of generic-class body in the whole
  compiler profile); `marked` **10 -> 8**; cost_gate PASS. Suite 20,275/0/44. **RESIDUE, measured and
  NOT closed**: `marked`'s remaining 8 are led by (CHK.35)'s TS7019/TS2683 family at 5 rows, and the
  TS2578 at `Instance.ts:179` is block-body return inference — neither is this family.

- [x] **(CHK.139) CLOSED 2026-09-21 ((P18.166) note) in BOTH halves — the READ arm (a UNION over the
  key's literals) and the WRITE slot (their INTERSECTION), plus the SINGLETON literal key the first
  cut missed and the intersection-display parenthesization tsgo prints. Suite 20,265/0/44, screen 0
  of 8,725, grid 8x0, cost_gate max +0.41%. **`marked` did NOT move and the reason is MEASURED, not
