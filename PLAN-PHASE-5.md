...
`EnumLiteralValueRelationTest`; no baseline moved and no `LogicalParityDivergence` is needed.
This round was ORCHESTRATED: one implementation subagent owned Gradle and landed (CHK.83)'s
two remaining measurable residues, while a READ-ONLY recon subagent, running against a
frozen snapshot of the HEAD binary (`Checker.class` `6e9e55a0…`), designed the (CHK.85)(a)
unblocker against both references — recorded as the new top-of-queue item (CHK.91). Every
row below was reproduced against tsgo 7.0.2 AND pristine `typescript@6.0.3` BEFORE any code
was written, and the two references agree on all of them.

**(CHK.83) item (1) LANDED — an enum MEMBER against a LITERAL target relates by VALUE.**
`const l1: 5 = em` (`em: E.A`, `A = 1`) was silent at the declaration, assignment, return and
argument positions; both references print `Type 'E.A' is not assignable to type '5'.` The
cause is one flag test: `isSimpleTypeRelatedTo`'s `EnumLiteral` leg accepted a member against
ANY number-literal target because `TypeFlags.NumberLike ⊇ NumberLiteral`. A
`Type.StringLiteral`/`Type.NumberLiteral` target now relates only through
`enumMemberValueEqualsLiteral`, which reads the member's value through `enumMemberEntries` —
the tsc-value view where an AMBIENT no-initializer member is OPAQUE — so `declare enum Amb
{ P }`'s `Amb.P` relates to no literal (both references agree; arm a2 proves `enumValues`,
which auto-numbers it, would have silently accepted `Amb.P` against `0`). Mixed enums, string
enums, a `2 | 3` union target and the whole-enum control all match both references.

**(CHK.83) item (2) LANDED — a LITERAL against an enum-flavoured target at every position.**
`fEnum(3)` (`E` has no member `3`) was silent at an argument while `const d: E = 3`
reported; round 745's literal-source helper (`enumNumericLiteralSource`, now
`enumTargetLiteralSource` + `typeHasEnumFlavoredConstituent`) is widened to STRING literals
(display-only by construction: a string enum's member set is what the relation already
decides) and to a UNION target carrying an enum, keeps the round-745 `undefined!`/`null`/
boolean firewall, and is wired at the return statement, the arrow expression body, the
argument gate, `checkRestArgsAgainstArrayElementType`, the three overload helpers (whose
TS2769 chain is now byte-identical to PRISTINE's per-candidate form), and the object-literal
member paths direct / nested / returned. The nested admission under `arrayPropsOnly` fires
only when the WIDENED type relates — the numeric case the whole relation cannot see — because
a string literal's widened `string` already fails the whole relation, and that path owns it
(the guard against a double emission). Beside it, `enumUnionTargetDisplay` now appends
`null` then `undefined` LAST (tsc's `formatUnionTypes`; HEAD printed `undefined | null | E`),
with `void` keeping its id place. **One edit was RETIRED by its own ablation**: an
`EnumMember` arm in `argPrimitiveVsCompositeParamCheckable` read 0 RED because
`isSimpleCheckableType` already admits `TypeFlags.EnumLiteral`, so a member parameter never
reaches that predicate — removed, and the comment records why.

**(CHK.83) item (3) STOPPED with its mechanism.** `checkRecursiveFunctionTypes` is a
corpus-unique suppress-and-reemit pin covering FOUR mechanisms of the `typeof fn`
self-referential family (three TS2769 arity false positives, `C.g(3)`'s TS2345, the TS2394
overload-vs-implementation row, and `f6("")`'s TS2345 with the `{ (): typeof f6; … }` display
plus TS2793 related info); the overload-set gate covers one of the four and lacks the
`typeof f6` rendering, so a unification is not surgical. (CHK.83) is CLOSED; its four
display/argument residues are queued as (CHK.92).

**(CHK.91) DESIGNED — the (CHK.85)(a) unblocker, measured on both references.** The
(P18.18) arm read its contextual KEEP off the PUSH field `contextualType`, which reaches an
object literal only from an OBJECT-target var-decl passing `objLitTargetNeedsContext` (no
enum arm in `propTypeContainsLiteral`), a return with an object / sole-non-nullish /
key-unique union target or a returned array, an argument with an object parameter, an
object-target assignment, and nested inheritance. Measured with an `f: (x) => 0` member
whose display reads `(x: number)` when per-property context reached and `(x: any)` when not:
the positions where an emitter fires for a widened discriminant AND no context reaches are
a UNION-annotated declaration, a literal nested under one, a CONDITIONAL return
(`findAllReferences.ts:1236`), an ARROW expression body and a union-target ASSIGNMENT —
exactly the eleven-loss class — while plain return / argument / `push` / `Map.set` are
silent for a widened discriminant either way, so the entry's `getUpToDateStatusWorker`
attribution cannot be where the `tsbuildPublic.ts` rows came from. **Two facts the arm did
not know**: tsc widens only a FRESH literal (an enum-member ACCESS expression; `{ v: a }`
with `a: K.A`, `{ v: k }` with `let k = K.A`, `{ v: K.A as K.A }` and every SHORTHAND stay
un-widened in both references), so the arm as built would have added two false positives
the grid had not yet reached; and the keep must be tsc's `isLiteralOfContextualType` (SOME
constituent carries a literal of the flavour), not `enumComparisonAtoms` (an EVERY test that
widens against `kind?: E.X`). The recommended shape is the arm's function unchanged + the
freshness gate + a PULL-derived `objLitMemberContextualType(prop)` mirroring tsc's
`getContextualType` roots, consulted only for enum-member-valued members (265 on the
compiler profile, ≤331 on any). Two further rows the entry lacked: `if (o.kind === K.B)` on
`const o = { kind: K.A }` is an ours-only TS2367, and the `never`-parameter probe is blind on
our side for an enum-member LOCAL (tsc 5 rows, ours 0) — grade (CHK.85)(b) with a primitive
mis-assignment. Designs (A) (a side fresh type) and (C) (widen at the read) are refused in
the item with their measurements.

**Pins and ablation.** 30 pins in `EnumLiteralValueRelationTest`, organised by rule with a
negative control per rule, full-text messages byte-identical to pristine, position pins.
Twelve arms, one mistake each, all discriminating: a1 the relation's flavour-only leg (**9
RED**); a2 `enumValues` for the tsc-value view (**1 RED**, the opaque-member pin alone); a3
the argument gate (**5**); a5 the union arm (**3**); a6 numeric-only (**2**); a7 the rest
element (**1**); a8 the overload literal read and a9 its chain display (**1** each, one pin,
nested per round 807); a10 the nullable order (**2**); a11 the direct-init objlit admission
(**2**); a12 the return statement and a13 the arrow body (**1** each, a pin asserting both
spellings). Source restored from a scratchpad snapshot (`cmp`-verified before arming and
after restoring; never `git checkout`), rebuilt, restored `Checker.class` sha256
`5d5e4319…` — the binary every gate ran against.

**Gates** (all against `5d5e4319`): core module **841 classes / 15,935 / 0 / 3**; generated
corpus **25 classes / 8,837 / 0**, every class present in the XMLs; full suite 17,424 / 0 / 3
across 961 result files; `cost_gate.py` **exit 0** (`output.errors` 46, `spine.nodes`
+0.00%; `typeOfExpr.calls` −0.21% / `globals.lookups` −0.49% are (P18.18)'s un-rebaselined
receipts carried forward, identical to (P18.19)'s reading); `huge_methods.py --fail-over 0`
**exit 0**; 8-profile grid with `PARITY1_MARKER=enumTargetLiteralSource`, BEFORE `6e9e55a0`
/ AFTER `5d5e4319`: **all eight `added=0 removed=0`** (harness 95 rows, the others 46).
Shipped builds 0 `w:`.

**Residues (all pre-existing on HEAD), queued as (CHK.92).** (a) An object-literal member
TARGET display is deliberately widened (`{ p: 5 }` prints `'number'`, both references print
the literal) — an earlier round's explicit choice at `emitPerPropertyMismatchesForObjectLiteral`.
(b) `fx({ e: 3 })`, the ARGUMENT-position object literal (`caasObjLitPerPropertyMismatch`), is
still silent. (c) An optional parameter `e?: E` prints `'E'` where both references print
`'E | undefined'` — tsc's `isRelatedTo` strips nullables only from a union of exactly ONE
non-nullable, and an enum is a union of its members; the same family as `gU(1)` printing
`'string | undefined'` where tsc prints `'string'`. (d) A SINGLE-member enum's member prints
`Cmp.X`/`CE.M` where both references print `Cmp`/`CE` (the parent's declared type IS the
member type). Trim-on-write: the (P18.9) note moved to `docs/history/PLAN-PHASE-5-HISTORY.md`.
