...
- `typeGuardConstructorClassAndNumber_ts` → MISS 8× TS2339 with multi-line chain `Property 'X' does not exist on type 'union member'.` in the negative branches (`var1.constructor != C1` else-body where `var1: C1 | number` keeps the union; `var1.property1` should fire TS2339 because `number` doesn't have it). Currently emits zero diagnostics — the union-receiver TS2339 with multi-member elaboration is not implemented. ~~Also needs ElementAccessExpression form (`var1["constructor"]`) for half the cases.~~ ElementAccess form landed 17.5b. Remaining gating piece: union-receiver TS2339 multi-member elaboration. **Skip-log analysis update (2026-04-25 post-17.5b)**: even with 17.5b's ElementAccess + negative-direction correction, the test still emits zero diagnostics because the property access on `number | C1` (negative branch keeps full union) is the diagnostic that should fire — and our `checkSinglePropertyAccess` returns at `if (exprType !is Type.Object) return` for `Type.Union` receivers (no multi-member union TS2339 walker). Unblocking this single test = implementing union-receiver TS2339 elaboration.
