...
- [ ] **17.34. Blocker #1 step 2h.iii — PropertyAccess narrowing wire-up (foundation laid 17.34a).** Substeps in order of expected impact and stacking dependency:
  ...
  - [ ] **17.34d. PropertyAccess narrowing in `getTypeOfPropertyAccess` for non-var-decl read positions.** Currently `getTypeOfPropertyAccess` returns the declared type without consulting the flow graph. Wiring this would make narrowed types visible in expression-context property reads (e.g. `console.log(A._a.length)` after `if (A._a)`). Risk: medium — every `.prop` read becomes a flow-graph lookup; performance impact needs measuring.
