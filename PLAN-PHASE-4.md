...
- [ ] **17.34. Blocker #1 step 2h.iii — PropertyAccess narrowing wire-up (foundation laid 17.34a).** Substeps in order of expected impact and stacking dependency:
  ...
  - [ ] **17.34e. Falsy-side narrowing for PropertyAccess (drop truthy union members on falsy branch).** `narrowByTruthiness(t, truthy = false)` currently returns `t` unchanged per the 17.7e session note's conservative gate. Once TypeScript-equivalent falsy-narrowing is implemented for Identifier (drops Type.NumberLiteral(0)/StringLiteral("")/etc. on the truthy side, drops everything-else on the falsy side), the same logic applies to PropertyAccess via the path comparison.
