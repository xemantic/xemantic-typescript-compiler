...
- [ ] **17.58. Blocker #5 — JSDoc `@type` parsing (~5–10 tests, LOW risk).** PRIORITIZED 2026-05-01 per user direction. The two concrete failing tests in the skip-log are `jsExportMemberMergedWithModuleAugmentation_ts` (MISS TS2564 for `class Abcde { /** @type {string} */ x; }` — needs `@type` annotation on a class property) and `jsFunctionWithPrototypeNoErrorTruncationNoCrash_ts` (MISS TS2339 for prototype-method `this.rgb()`). Decompose smallest-first:
  - [ ] **17.58a. Scanner-level JSDoc capture.** Currently the scanner skips comments. Need to retain `/** ... */` content as either a SyntaxKind or attached to the next-token's leading trivia, addressable by the parser. tsgo reference: `internal/parser/jsdoc.go`. Smallest scope: just emit a `JSDocCommentTrivia` annotation on tokens that follow a `/** ... */` block, no parsing yet.
  - [ ] **17.58b. Parser bridge for `@type {T}` on PropertyDeclaration.** When a PropertyDeclaration has no explicit `type` annotation but its leading trivia includes `/** @type {T} */`, parse the inner type expression as a regular TypeNode and attach it as `decl.type`. Conservative: only fire in `.js`/`.jsx` files. Scope to PropertyDeclaration first (the `jsExportMemberMergedWithModuleAugmentation` case); follow-up can extend to ParameterDeclaration / VariableDeclaration / FunctionDeclaration return type.
  - [ ] **17.58c. Verify TS2564 fires.** The existing TS2564 walker at `checkUsesOfUninitialized` looks at `decl.type` to decide whether the property has a type annotation. With 17.58b's `@type` bridge, the `class Abcde { /** @type {string} */ x; }` declaration should now have `decl.type = StringKeyword`, triggering the standard TS2564 emission. Verify; flip the test.

  **Yield expectation:** 17.58a is foundation (net-zero). 17.58b on PropertyDeclaration unblocks `jsExportMemberMergedWithModuleAugmentation_ts` (+1). Extending to other declaration positions in follow-ups can unlock additional `.js` tests in the corpus (~5–10 total per the Blocker #5 estimate). The work is contained to scanner + parser; no checker changes for the basic cases. Risk is LOW because changes are scoped to JS files only — TS files are untouched.
