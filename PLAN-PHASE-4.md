...
- [ ] **17.32. Blocker #3 step 1 — Cross-file global scope conflation, infrastructure-only sub-step (MEDIUM-HIGH yield ~30+ tests, HIGHEST risk).** Per the blocker description, recommended approach is "introduce per-file-scope construction behind a flag, run side-by-side, then flip per file." Smallest substep:
  ...
  - **17.32e+ (queued):** continue flipping identifier-resolution call sites to consult `perFileScope` instead of the merged `globals` map. Remaining: TS2304 visibility check at file-root (highest blast radius — defer until other-file scoped consumers prove stable). Note that the file-root NameScope built at `checkUnresolvedNames` (Checker.kt:8587) still includes `globals` for TS2304 — only the TS2552 suggestion sub-pool has been narrowed.
