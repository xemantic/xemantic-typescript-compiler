...
- [ ] **17.34. Blocker #1 step 2h.iii — PropertyAccess narrowing wire-up (foundation laid 17.34a).** Substeps in order of expected impact and stacking dependency:
  ...
  - [ ] **17.34b. Extend other narrowing operators (`narrowByEquality`, `narrowByInstanceOf`, `narrowByInOperator`, `narrowByCallPredicate`, `narrowByConstructorEquals`) to compare PropertyAccess paths.** All five currently use `expr is Identifier && expr.text == name`-pattern checks (Checker.kt:36175, 36241, 36312, 36342, 31229). Replace with a `getReferencePath(expr) == name` check using the helper from 17.34a. Risk: low; identity-based path comparison preserves existing Identifier behavior. May flip `if (A._a === 'foo') { ... A._a ... }` patterns that currently bail because `expr.left` is PropertyAccess.
