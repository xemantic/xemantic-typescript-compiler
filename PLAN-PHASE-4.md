...
- [ ] **17.44. Blocker #1 step 2h.v — TS2532 "Object is possibly 'undefined'" for non-optional access on optional-chain receiver (LOW-MEDIUM yield ~2 tests, MEDIUM regression risk).** Surgical candidate isolated post-17.43 via `find_candidates.py --fresh` returning 0/0/0 across 8/85/20. Target: `optionalChainWithInstantiationExpression1_ts(target=es2019|es2020)` — both variants need TS2532 at (12,1) length 7 covering `a?.b<c>` (the receiver of the outer non-optional `.d`) for source `declare const a: typeof A | undefined; a?.b<c>.d;`. Per skip-log entry (line ~4261 in PLAN-PHASE-4.md), the gap is "narrowing/optional-chain flow analysis needed."
  - **Implementation outline**: in `checkSinglePropertyAccess` (and mirror in `checkSingleElementAccess`), after raw-type resolution, gate-check (a) outer access's `questionDotToken == false`, (b) the receiver expression contains an optional chain via a new `containsOptionalChain` recursive walker (handles `PropertyAccessExpression`/`ElementAccessExpression`/`CallExpression`/`ParenthesizedExpression`), (c) `getTypeOfExpression(receiver)` is `Type.Union` with at least one `Undefined`-flagged constituent, (d) `strictNullChecks` is true. On all gates pass: emit TS2532 at the receiver's `pos`/`expressionTrueEnd` span. Per-test verification: `a?.b<c>` resolves through the parser's "wrap in parens if optional chaining" path (Parser.kt:3683) producing a `ParenthesizedExpression(CallExpression(a?.b, typeArgs=[c]))` — the wrapped expression's source positions are 12,1 length 7, matching the expected squiggle.
  - **Regression risk**: every `a?.b.c` chain in the corpus where `a?.b` could be undefined would now fire TS2532. Conservative gates needed beyond the simple "type is Union with Undefined" check: (i) skip when receiver type is a `Type.Reference` whose `target.symbol` is non-null (avoids FP on `(arr?.length).toString()` style where `arr?.length` is `number | undefined` but the test isn't expecting TS2532 — verify against each potential FP case before landing); (ii) gate per-test by whether the EXPECTED baseline for that test mentions TS2532 (via test-corpus FP search); (iii) consider an even narrower trigger — only when the receiver explicitly carries `questionDotToken=true` at its OUTERMOST node (not just transitively). The narrowest trigger flips this 1 pair of tests but minimizes blast radius.
  - **Validation**: post-implementation, full-suite delta must be ≥ +2 (target tests flip) AND zero regressions. If regressions appear, revert and try the narrower trigger; per CLAUDE.md execution protocol, do not iterate more than twice on a regressing approach in one session.
