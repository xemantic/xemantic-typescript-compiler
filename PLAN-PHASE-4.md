...
- [ ] **17.31. Blocker #2 step 1 — Generic argument inference, smallest substep (HIGH yield ~20–40 tests, MEDIUM risk).** Stacks on the 16.4 generic infrastructure. Smallest-substep decomposition:
  ...
  - [ ] **17.31e. Inference from generic type arguments (`Array<T>` from `[1,2]`).** Extends 17.31a–c/d into non-rest `Type.Reference` arg shapes — match source's resolvedTypeArguments to target's TypeParam positions.
