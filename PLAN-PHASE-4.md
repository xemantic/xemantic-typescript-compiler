...
- [ ] **17.46. Blocker #1 step 2h.vi (retry) — Pre-installed type-guard recognition for top-level walker (broad-surface decomposition).** Path (a) per 17.45's forensics: batch-add the missing type-guard patterns to `conditionImpliesAssignedTrue` BEFORE re-attempting the top-level walker, so each substep lands as a clean standalone commit. Substeps:
  ...
  - [ ] **17.46b. Extend `isUnresolvedGenericType` to QualifiedName receivers.** Filter for `var b: A.B` where `A.B` is a TS2314 generic missing type args. Currently the predicate only checks Identifier name (Checker.kt:17121-17133), so `A.B` falls through and `b` is added to the uninitialized set. Without this, the future top-level walker FP-emits TS2454 on `genericCloduleInModule2_ts`-style cases.
