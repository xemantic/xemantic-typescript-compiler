...
**Session 2026-04-26 (17.10a–d, 8361 → 8367, +6) — type-parameter wiring on FunctionExpression / FunctionType / TypeLiteral call sigs:** Surgical session focused on the `assignmentCompatability24/33/34_ts` family + adjacent FunctionType-as-target tests. Four sub-commits:
...
- **17.10d (net-zero infra)**: Light type-param "inference" in `signatureRelatedTo` for source-generic + target-non-generic case. Source's TypeParam-typed param positions are pinned to the target's concrete type at that position; same TypeParam appearing multiple times must map to the SAME target type (ref-equality); each pinning checks the constraint. Fixes the FP-rejection of `(g: Giraffe) => void` ← `<T extends Animal>(x: T) => void` introduced by 17.10b — without this rule, signatureRelatedTo's standard contravariant check blanket-fails "concrete vs TypeParam" pairs. Net-zero because the targeted test (`contextualSignatureInstatiationContravariance_ts`) still fails on the line-8 elaboration content (expects "Property 'y' is missing in Elephant but required in Giraffe" via type-arg substitution — full inference).

**Explored-but-skipped this session:**
- `contextualSignatureInstatiationContravariance_ts` — needs full type-arg inference (substitute T = first-pinned-type into elaboration chain). After 17.10d the FP line-11 is gone but line-8 elaboration is still wrong: we emit "Types of parameters 'x' and 'g' are incompatible. Type 'Giraffe' is not assignable to type 'T'." while TypeScript emits "Types of parameters 'y' and 'e' are incompatible. Property 'y' is missing in type 'Elephant' but required in type 'Giraffe'." — the latter requires substituting T→Giraffe (from the first pair) then reporting the property-missing chain on the SECOND pair. Pure Blocker #2 territory.
- `genericFunctionCallSignatureReturnTypeMismatch_ts` / `functionTypeArgumentAssignmentCompat_ts` — close after 17.10b's display fix but need `S→unknown` widening (when source has an unconstrained TypeParam in return type) plus the "TypeParam could be instantiated with arbitrary type" elaboration line. Two pieces, deferred.
- `typeAssertionToGenericFunctionType_ts` — MISS TS2554 at `x.b<string>()` where `x.b` is an arrow function in an object literal. Our TS2554 walker (`checkArgCountInExpr`) only handles direct identifier and `new` calls — extending to PropertyAccessExpression callees is broader scope (would need to resolve the property's type to a callable, then check arg count). Single-test gain, not pursued this session.
...
- `arrayAssignmentTest4_ts` → SWAP TS2322 / TS2740 for ctor-assigned class vs array-like. Needs function-vs-array assignability refinement.
