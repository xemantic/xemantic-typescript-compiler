...
- [ ] **17.30. Blocker #1 step 2h — Full control flow narrowing remaining wire-ups (HIGHEST yield ~60–100 tests, MEDIUM risk).** Flow-graph infra landed 17.1a–17.7. Remaining substeps, smallest-first:
  ...
  - [ ] **17.30c. `&&`-chain NARROWING wired into TS2774 emission. BLOCKED-PENDING-USER on lib.dom.d.ts loading.** Recon 2026-04-26 (post-17.30b session): all currently-failing TS2774 candidates that would benefit from `&&`-chain narrowing are wall-to-wall blocked on `window` resolving to a real DOM type rather than `anyType`. Sweep across `truthinessCallExpressionCoercion{1..4}_ts`, `truthinessPromiseCoercion_ts`, `uncalledFunctionChecksInConditional{,2,Perf}_ts` confirms: the only failing TS2774 cases are `window.performance` / `window.console.error` / `window.matchMedia` / `inBrowser && window.performance` chains in `uncalledFunctionChecksInConditional2_ts` and the missing 35th emission in `truthinessCallExpressionCoercion2_ts` (currently 34/35) — both gated on `window` resolving past `anyType`. Implementing `&&`-chain narrowing in isolation flips ZERO tests. The unblocker (lib.dom.d.ts loading or a hand-rolled `Window` interface in BUILTIN_LIB_SOURCE) is itself a significant infrastructure piece — out of scope per autonomous-decision policy until user prioritizes. Recommendation: pursue 17.31 (generic argument inference) or 17.32 (cross-file scope) before returning to 17.30c with the `window` unblocker.
