...
- [ ] **17.45. Blocker #1 step 2h.vi — TS2454 flow-graph walker for top-level body reads + `Array.isArray(varName)` type-guard recognition (+1 — flips `isArray_ts`).** Gap: 17.30a's `checkDefiniteAssignmentViaFlowGraph` is gated to function/method bodies (`walkTopForFlowTS2454` only handles `FunctionDeclaration`/`ClassDeclaration`/`ModuleDeclaration`). For top-level `var x: T;` followed by `if (cond) { ... } else { x.foo }`, the legacy `checkUsesOfUninitialized` skips IfStatement bodies and the flow-graph walker doesn't run. Companion gap: `Array.isArray(varName)` true-branch implies `varName` is assigned (TypeScript treats successful type predicates as assignment evidence) but `conditionImpliesAssignedTrue` only recognizes `typeof`/`!= null`/bare-truthy/`&&`. Two-piece change: (1) New `runFlowTS2454OnTopLevel` mirrors `runFlowTS2454OnFunction` but for the file's top-level statements — collects file-level uninit vars via `collectAllUninitVarsInFunction`, walks each top-level stmt with `walkStmtForFlowTS2454(s, uninit, ..., inUncheckedBody=false)`. Called from `checkDefiniteAssignmentViaFlowGraph` after the existing per-binderResult function-walk. Dedup via the existing `emittedTs2454Positions` set prevents double-fire with the legacy walker's top-level emissions. (2) `conditionImpliesAssignedTrue` adds a CallExpression branch matching exactly `Array.isArray(<Identifier>)` (PropertyAccess `Array`/`isArray`, single arg = Identifier with text == varName). Conservative scope — does NOT generalize to other type predicates or to `obj.method(varName)` patterns; the corpus's `isArray_ts` is the single test that gates on this. Risk: extending the flow-graph walker to top-level may FP on tests with patterns where some user-defined function is a type predicate that we don't recognize — narrow risk per the existing `inUncheckedBody` gate (only emits inside if/while/for/switch/try bodies, not top-level reads handled by the ad-hoc walker).
