...
- [ ] **17.30. Blocker #1 step 2h — Full control flow narrowing remaining wire-ups (HIGHEST yield ~60–100 tests, MEDIUM risk).** Flow-graph infra landed 17.1a–17.7. Remaining substeps, smallest-first:
  ...
  - [ ] **17.30c. `&&`-chain NARROWING wired into TS2774 emission.** UNBLOCKED 2026-05-01 (17.57): hand-rolled `Window` / `Performance` / `MediaQueryList` interfaces added to `BUILTIN_LIB_SOURCE` per user authorization. Now `window.performance` resolves to `Performance` and `window.console.error` to a non-optional function. Next session can wire `&&`-chain narrowing into TS2774 emission (the original 17.30c spec) for `perf && perf.measure && ...` patterns. Original recon (2026-04-26 post-17.30b): all currently-failing TS2774 candidates that would benefit from `&&`-chain narrowing were wall-to-wall blocked on `window` resolving past `anyType` — sweep across `truthinessCallExpressionCoercion{1..4}_ts`, `truthinessPromiseCoercion_ts`, `uncalledFunctionChecksInConditional{,2,Perf}_ts` confirmed the only failing TS2774 cases were `window.performance` / `window.console.error` / `window.matchMedia` chains in `uncalledFunctionChecksInConditional2_ts` and the missing 35th emission in `truthinessCallExpressionCoercion2_ts`.
