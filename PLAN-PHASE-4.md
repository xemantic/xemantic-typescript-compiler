...
- [ ] **17.46. Blocker #1 step 2h.vi (retry) — Pre-installed type-guard recognition for top-level walker (broad-surface decomposition).** Path (a) per 17.45's forensics: batch-add the missing type-guard patterns to `conditionImpliesAssignedTrue` BEFORE re-attempting the top-level walker, so each substep lands as a clean standalone commit. Substeps:
  ...
  - [ ] **17.46c. Extend `bindAssignmentTarget` to ArrayLiteralExpression / ObjectLiteralExpression destructuring.** Currently Flow.kt:697 only handles Identifier / ArrayBindingPattern / ObjectBindingPattern / PropertyAccess / ElementAccess as assignment targets. For `[, nameA] = robotA` the LHS is an ArrayLiteralExpression — falls into the else branch, doing nothing → no FlowAssignment for nameA. Multiple `sourceMapValidationDestructuring*` tests would FP-emit TS2454 from the future top-level walker without this.
