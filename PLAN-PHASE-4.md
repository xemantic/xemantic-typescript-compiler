...
- [ ] **17.34. Blocker #1 step 2h.iii — PropertyAccess narrowing wire-up (foundation laid 17.34a).** Substeps in order of expected impact and stacking dependency:
  ...
  - [ ] **17.34c. Wire PropertyAccess narrowing into TS2339 union-receiver narrowed-to-never branch (Checker.kt:42376).** Currently gated on `objectExpr is Identifier`. Extending to `objectExpr is PropertyAccessExpression || ...` would flip cases like `if (typeof obj.prop === 'string' || typeof obj.prop === 'number') {} else { obj.prop.bar; }` (`obj.prop` narrowed to `never` in else branch via exhaustive typeof-narrowing). Risk: medium — could over-narrow if the expression has side-effects between the guard and the access (TypeScript's invariant: only narrow stable references; PropertyAccess on a mutable object isn't strictly stable).
