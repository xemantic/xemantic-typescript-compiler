...
- [ ] **17.32. Blocker #3 step 1 — Cross-file global scope conflation, infrastructure-only sub-step (MEDIUM-HIGH yield ~30+ tests, HIGHEST risk).** Per the blocker description, recommended approach is "introduce per-file-scope construction behind a flag, run side-by-side, then flip per file." Smallest substep:
  ...
  - **17.32c+ (queued by 17.32b's session note):** continue flipping identifier-resolution call sites to consult `perFileScope` instead of the merged `globals` map. Remaining order: TS2552 spelling suggestion candidate set (medium risk — touches the suggestion candidate pool which is broader than the visible-binding check), default-import-from-export-equals visibility (narrow — single-pattern resolver helper), TS2304 (highest blast radius — defer).
