...
class Transformer(
    ...
    // member name (incl. leading '#') to its downlevel entry, or null for a '#' member that
    // is declared but NOT downleveled (the null blocks an outer-class member with the same
    // name from being wrongly matched). Pushed per class in transformClassBody when
    ...
    private fun lookupPrivateEnv(name: String): PrivateEnvEntry? {
    ...
    private fun transformToAMD(
        ...
        // Counter for generating unique import-alias temp names (per-file).
        val moduleNameCounter = mutableMapOf<String, Int>()
    ...
    private fun makeExportAssignment(name: String, value: Expression? = null, leadingComments: List<Comment>? = null, pos: Int = -1, trailingComments: List<Comment>? = null): Statement {
        return ExpressionStatement(
            expression = BinaryExpression(
                left = PropertyAccessExpression(
                    expression = syntheticId("exports"),
    ...
    private fun transformExpression(expr: Expression): Expression {
        return when (expr) {
            ...
            is CallExpression -> {
                ...
                run {
                    val pCallee = expr.expression as? PropertyAccessExpression ?: return@run
                    if (!pCallee.name.text.startsWith("#") || pCallee.expression !is Identifier) return@run
                    ...
                    return CallExpression(
                        expression = PropertyAccessExpression(
                            expression = CallExpression(
                                ...
                                arguments = listOf(
                                    recv,
                                    syntheticId(entry.brandVar),
                                    StringLiteralNode(text = "m", pos = -1, end = -1),
                                    syntheticId(entry.stateVar),
                                ),
                                pos = -1, end = -1,
                            ),
                        ...
                        arguments = listOf(recv) + expr.arguments.map { transformExpression(it) },
                        pos = -1, end = -1,
                    )
                }
            ...
            is PropertyAccessExpression -> {
                ...
                // downleveled the field to a WeakMap; a captured private METHOD reads as
                // `__classPrivateFieldGet(expr', _brand, "m", _m)` (B341).
                if (expr.name.text.startsWith("#")) {
                    val entry = lookupPrivateEnv(expr.name.text)
                    if (entry != null) {
                        requireHelper("__classPrivateFieldGet")
                        val args = if (entry.kind == 'm' && entry.brandVar != null) listOf(
    ...
    private fun transformClassBody(
        ...
        val privateEnv = mutableMapOf<String, PrivateEnvEntry?>()
        if (needsPrivateFieldDownlevel) {
            ...
            for (info in privateFieldInfos) privateEnv["#${info.fieldName}"] = PrivateEnvEntry('f', info.weakMapVar)
            // B341: captured private methods — calls/reads route through the brand WeakSet.
            brandVar?.let { bv ->
                    ...
                    val lhs: Expression? = when {
                        ...
                        prop.name is ComputedPropertyName -> ElementAccessExpression(
                            ...
                            argumentExpression = computedPropTempVars[prop]?.let { syntheticId(it) }
                                ?: transformExpression(prop.name.expression),
                            pos = -1, end = -1,
                        )
                        else -> null
                    }
    ...
    /**
     * Replace a computed property name expression with a temp var reference.
     * Used for computed property name extraction.
     */
    private fun replaceComputedNameWithTemp(element: ClassElement, tempVar: String): ClassElement {
        val tempName = ComputedPropertyName(
            expression = syntheticId(tempVar),
            pos = -1, end = -1,
        )
        return when (element) {
            is MethodDeclaration -> element.copy(name = tempName)
            is GetAccessor -> element.copy(name = tempName)
            is SetAccessor -> element.copy(name = tempName)
            is PropertyDeclaration -> element.copy(name = tempName)
            else -> element
        }
    }
